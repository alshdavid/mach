...
fn make_field_enum_variant_from_named_field(
  type_name: &Ident,
  f: &Field,
) -> Variant {
...

    {
      ...
      optional_methods.push(ImplItemFn {
        ...
        block: match mode {
          ...
          Mode::Visit(VisitorVariant::WithPath) | Mode::VisitMut(VisitorVariant::WithPath) => {
            parse_quote!({
                if self.enabled {
      ...
      Mode::Fold(VisitorVariant::Normal) => quote!(
          ...
          T: 'static + FoldWith<V>,
        {
          fn fold_with(
            ...
            ::swc_visit::util::map::Map::map(self, |value| <T as FoldWith<V>>::fold_with(value, v))
          }
      ...
      Mode::VisitMut(VisitorVariant::WithPath) => quote!(
          ...
          T: 'static + VisitMutWithPath<V>,
        {
          ...
          fn visit_mut_children_with_path(
            ...
            <T as VisitMutWithPath<V>>::visit_mut_children_with_path(&mut **self, v, ast_path);
          }
        }
      ),
    };
  ...
  adjust_expr(mode, ty, expr, |expr| match mode {
    ...
    | Mode::Visit(VisitorVariant::WithPath) => {
      if let Some((type_name, field_name)) = ast_path {
        ...
        let field_name = Ident::new(&field_name.to_string().to_pascal_case(), field_name.span());
        ...
        if let Some(arg) = extract_generic("Option", ty) {
          ...
          if let Some(item) = extract_generic("Vec", arg) {
            match mode {
              ...
              Mode::Visit(VisitorVariant::Normal) | Mode::VisitAll => {
                return mk_exact(mode, ident, &parse_quote!(::std::option::Option<&[#item]>));
              }
              Mode::Visit(VisitorVariant::WithPath) => {
                return mk_exact(
                  mode,
          ...
          match mode {
            Mode::Fold { .. } => {
              return mk_exact(mode, ident, &parse_quote!(::std::option::Option<#arg>));
            }
            Mode::VisitMut { .. } => {
              return mk_exact(mode, ident, &parse_quote!(&mut ::std::option::Option<#arg>));
            }
            Mode::Visit(VisitorVariant::Normal) | Mode::VisitAll => {
              return mk_exact(mode, ident, &parse_quote!(::std::option::Option<&#arg>));
            }
            ...
            PathArguments::AngleBracketed(tps) => {
              ...
              match arg {
                GenericArgument::Type(arg) => {
                  ...
                  match mode {
                    Mode::Fold { .. } => {
                      return mk_exact(mode, ident, &parse_quote!(::std::vec::Vec<#arg>));
                    }
                    Mode::VisitMut { .. } => {
                      return mk_ref(mode, ident, &parse_quote!(::std::vec::Vec<#arg>), true);
                    }
                    Mode::Visit { .. } | Mode::VisitAll => {
                      return mk_ref(mode, ident, &parse_quote!([#arg]), false);
                    }
                  }
                }
        ...
        if let Some(arg) = extract_box(ty) {
          match mode {
            Mode::Fold(..) => {
              ...
              let inner = inject_ast_path_arg_if_required(mode, parse_quote!(_visitor.#method(*n)));
                ...
                GenericArgument::Type(arg) => {
                  ...
                  if let Mode::Fold(..) = mode {
                    if extract_box(arg).is_some() {
                      let inner =
                        inject_ast_path_arg_if_required(mode, parse_quote!(_visitor.#method(n)));
...
fn inject_ast_path_arg_if_required(
  mode: Mode,
