...
use self::text_writer::WriteJs;
use self::util::StartsWithAlphaNum;
  ...
  S: SourceMapperExt,
{
  ...
  #[emitter]
  fn emit_namespace_export_specifier(
    &mut self,
  ...
  #[emitter]
  fn emit_named_export_specifier(
    &mut self,
      ...
      match &num.raw {
        Some(raw) => {
          if raw.len() > 2 && self.cfg.target < EsVersion::Es2015 && {
            ...
            self.wr.write_str_lit(DUMMY_SP, &value)?;
          } else if raw.len() > 2 && self.cfg.target < EsVersion::Es2021 && raw.contains('_') {
  ...
  fn emit_class_trailing(
    ...
    if node.super_class.is_some() {
      ...
      {
        let starts_with_alpha_num = node.super_class.as_ref().unwrap().starts_with_alpha_num();
      ...
      PropName::Ident(ident) => {
        ...
        if self.cfg.ascii_only {
          if self.wr.can_ignore_invalid_unicodes() {
            ...
            self.wr.write_symbol(
              DUMMY_SP,
              &get_ascii_only_ident(&handle_invalid_unicodes(&ident.sym), true, self.cfg.target),
            )?;
          }
        } else {
          emit!(ident);
  ...
  fn emit_template_for_tagged_template(
    &mut self,
    node: &Tpl,
  ) -> Result {
    debug_assert!(node.quasis.len() == node.exprs.len() + 1);
  ...
  fn emit_template_element_for_tagged_template(
    &mut self,
    node: &TplElement,
  ) -> Result {
    srcmap!(self, node, true);
  ...
  fn emit_ident(
    ...
    if self.cfg.ascii_only {
      if self.wr.can_ignore_invalid_unicodes() {
        ...
        self.wr.write_symbol(
          DUMMY_SP,
          &get_ascii_only_ident(&handle_invalid_unicodes(&ident.sym), false, self.cfg.target),
        )?;
      }
  ...
  S: SourceMapperExt,
{
  ...
  fn simple_assign_target_has_leading_comment(
    &self,
    arg: &SimpleAssignTarget,
  ) -> bool {
    match arg {
  ...
  #[emitter]
  pub fn emit_module_export_name(
    &mut self,
    ...
    match unescape {
      ...
      Some(c) => {
        if !ascii_only || c.is_ascii() {
          ...
          buf.extend(
            c.escape_unicode()
              .map(|c| if c == 'u' { c } else { c.to_ascii_uppercase() }),
          );
        }
      }
      None => {}
            ...
            if let Some(c @ 'D' | c @ 'd') = next {
              ...
              if is_valid {
                ...
                let v = u32::from_str_radix(val_str, 16).unwrap_or_else(|err| {
                  unreachable!("failed to parse {} as a hex value: {:?}", val_str, err)
                });
