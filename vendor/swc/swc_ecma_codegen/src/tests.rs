...
use self::swc_ecma_parser::EsConfig;
use self::swc_ecma_parser::Parser;
use self::swc_ecma_parser::StringInput;
use self::swc_ecma_parser::Syntax;
use super::*;
...
#[track_caller]
pub(crate) fn assert_min_target(
  from: &str,
  to: &str,
  target: EsVersion,
) {
...
#[track_caller]
pub(crate) fn assert_min_typescript(
  from: &str,
  to: &str,
) {
  let out = parse_then_emit(
