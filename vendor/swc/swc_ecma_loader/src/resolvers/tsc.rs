  ...
  fn resolve(
    ...
    if let FileName::Real(v) = base {
      if v.components().any(|c| match c {
        ...
        return self
          .invoke_inner_resolver(base, module_specifier)
          .context("not processed by tsc resolver because base module is in node_modules");
      }
    }
      ...
      match from {
        Pattern::Wildcard { prefix } => {
          ...
          for target in to {
            ...
            let res = self
              .invoke_inner_resolver(base, module_specifier)
              .or_else(|_| self.invoke_inner_resolver(&self.base_url_filename, &relative))
              .or_else(|_| self.invoke_inner_resolver(&self.base_url_filename, &replaced));
