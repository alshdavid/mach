...
use swc_ecma_utils::is_valid_prop_ident;
...
use self::constructor::constructor_fn;
use self::constructor::make_possible_return_value;
use self::constructor::replace_this_in_constructor;
use self::constructor::ConstructorFolder;
use self::constructor::ReturningMode;
use self::constructor::SuperCallFinder;
use self::constructor::SuperFoldingMode;
use self::prop_name::is_pure_prop_name;
use self::prop_name::should_extract_class_prop_key;
use self::prop_name::HashKey;
  ...
  C: Comments,
{
  ...
  fn visit_mut_assign_pat(
    ...
    if let (Pat::Ident(BindingIdent { id, .. }), Expr::Class(c @ ClassExpr { ident: None, .. })) =
      (&*n.left, &mut *n.right)
    {
  ...
  fn class_to_stmts(
    ...
    for member in class.body {
      match member {
        ...
        ClassMember::PrivateProp(..) => {
          unreachable!("classes pass: private property\nclass_properties pass should remove this")
        }
...

    {
      ...
      if super_class_ident.is_some() {
        ...
        insert_this |=
          (mode.is_none() && !is_always_initialized) || mode == Some(SuperFoldingMode::Assign);
      ...
      let is_this_declared =
        (insert_this && super_class_ident.is_some()) || (mode == Some(SuperFoldingMode::Var));
