...
use swc_ecma_utils::function::init_this;
          ...
          unpack_rest = Some(Stmt::For(ForStmt {
                ...
                decls: vec![
                  ...
                  VarDeclarator {
                    ...
                    init: Some(Box::new(Expr::New(NewExpr {
                      span,
                      callee: Box::new(
                        quote_ident!(DUMMY_SP.with_ctxt(self.unresolved_ctxt), "Array").into(),
                      ),
                      args: Some(vec![{
                        // `len` or  `len - $i`
            ...
            body: Box::new(Stmt::Block(BlockStmt {
              ...
              stmts: vec![{
                ...
                AssignExpr {
                  span,
                  left: arg.computed_member(make_minus_i(&idx_ident, false)).into(),
                  op: op!("="),
                  right: Box::new(
                    MemberExpr {
                      span: DUMMY_SP,
                      obj: Box::new(quote_ident!(span, "arguments").into()),
                      prop: MemberProp::Computed(ComputedPropName { span, expr: prop }),
                    }
                    .into(),
                  ),
                }
