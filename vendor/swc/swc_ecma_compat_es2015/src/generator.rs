...
use swc_ecma_utils::function::FnEnvHoister;
use swc_ecma_utils::private_ident;
use swc_ecma_utils::prop_name_to_expr_value;
...
impl VisitMut for Generator {
    ...
    match e {
      ...
      Expr::Cond(node) => {
        ...
        if contains_yield(&node.cons) || contains_yield(&node.alt) {
          ...
          self.emit_assignment(result_local.clone().into(), node.alt.take(), Some(alt_span));
      ...
      Expr::Assign(node) if contains_yield(&node.right) => {
        ...
        if node.op != op!("=") {
          let left_of_right = self.cache_expression(node.left.take().expect_simple().into());
              ...
              PropOrSpread::Prop(p) => match *p {
                Prop::Getter(p) => {
                  if let Some(CompiledProp::Accessor(g, _)) =
                    props.iter_mut().find(|prev| match prev {
                      CompiledProp::Accessor(_, Some(s)) => s.key.eq_ignore_span(&p.key),
                      _ => false,
                    })
                  {
                    *g = Some(p);
                ...
                Prop::Setter(p) => {
                  if let Some(CompiledProp::Accessor(_, s)) =
                    props.iter_mut().find(|prev| match prev {
                      CompiledProp::Accessor(Some(prev), _) => prev.key.eq_ignore_span(&p.key),
                      _ => false,
                    })
                  {
                    *s = Some(p);
  ...
  fn visit_mut_call_expr(
    ...
    if !node.callee.is_import() && node.args.iter().any(contains_yield) {
      ...
      let (target, this_arg) = self.create_call_binding(node.callee.take().expect_expr(), false);
...
impl Generator {
  ...
  fn visit_left_associative_bin_expr(
    &mut self,
    node: &mut BinExpr,
  ) -> Option<Expr> {
  ...
  fn transform_and_emit_embedded_stmt(
    &mut self,
    node: Stmt,
  ) {
  ...
  fn transform_and_emit_stmt_worker(
    &mut self,
    mut node: Stmt,
  ) {
    match node {
  ...
  fn transform_and_emit_expr_stmt(
    &mut self,
    mut node: ExprStmt,
  ) {
    node.visit_mut_with(self);
  ...
  fn transform_and_emit_var_decl_list(
    &mut self,
    mut node: Box<VarDecl>,
  ) {
  ...
  fn transform_and_emit_do_stmt(
    &mut self,
    mut node: DoWhileStmt,
  ) {
    if contains_yield(&node) {
      // [source]
      //      do {
  ...
  fn transform_and_emit_while_stmt(
    &mut self,
    mut node: WhileStmt,
  ) {
  ...
  fn transform_and_emit_for_stmt(
    &mut self,
    mut node: ForStmt,
  ) {
    if contains_yield(&node) {
  ...
  fn transform_and_emit_for_in_stmt(
    &mut self,
    mut node: ForInStmt,
  ) {
    if contains_yield(&node) {
  ...
  fn transform_and_emit_continue_stmt(
    &mut self,
    node: ContinueStmt,
  ) {
  ...
  fn transform_and_emit_break_stmt(
    &mut self,
    node: BreakStmt,
  ) {
  ...
  fn transform_and_emit_return_stmt(
    &mut self,
    mut s: ReturnStmt,
  ) {
  ...
  fn transform_and_emit_with_stmt(
    &mut self,
    mut node: WithStmt,
  ) {
    if contains_yield(&node) {
  ...
  fn transform_and_emit_switch_stmt(
    &mut self,
    mut node: SwitchStmt,
  ) {
  ...
  fn transform_and_emit_labeled_stmt(
    &mut self,
    mut node: LabeledStmt,
  ) {
    #[cfg(debug_assertions)]
  ...
  fn transform_and_emit_throw_stmt(
    &mut self,
    mut node: ThrowStmt,
  ) {
    node.arg.visit_mut_with(self);
  ...
  fn transform_and_emit_try_stmt(
    &mut self,
    mut node: TryStmt,
  ) {
  ...
  fn count_initial_nodes_without_yield<N>(
    &self,
    nodes: &[N],
  ) -> usize
  where
  ...
  /// labels.
  fn supports_labeled_break_or_continue(
    &self,
    block: &CodeBlock,
  ) -> bool {
  ...
  fn supports_unlabeled_continue(
    &self,
    block: &CodeBlock,
  ) -> bool {
  ...
  fn has_immediate_containing_labeled_block(
    &self,
    label_text: &JsWord,
    start: usize,
  ) -> bool {
        ...
        match &*b {
          CodeBlock::Exception(_) => {
            if block_action == BlockAction::Open {
              ...
              self.current_exception_block = self.exception_block_stack.as_mut().unwrap().pop();
              #[cfg(debug_assertions)]
              debug!(
