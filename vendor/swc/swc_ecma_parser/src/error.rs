...
impl SyntaxError {
  ...
  pub fn msg(&self) -> Cow<'static, str> {
    match self {
      SyntaxError::PrivateNameInInterface => "private names are not allowed in interface".into(),
      SyntaxError::TopLevelAwaitInScript => "top level await is only allowed in module".into(),
      SyntaxError::LegacyDecimal => "Legacy decimal escape is not permitted in strict mode".into(),
      SyntaxError::LegacyOctal => "Legacy octal escape is not permitted in strict mode".into(),
      ...
      SyntaxError::LegacyCommentInModule => "Legacy comments cannot be used in module code".into(),
      ...
      SyntaxError::UnaryInExp { .. } => "'**' cannot be applied to unary/await expression.".into(),
      ...
      SyntaxError::MultipleDefault { .. } => "A switch block cannot have multiple defaults".into(),
      ...
      SyntaxError::VarInitializerInForInHead => "Unexpected initializer in for in/of loop".into(),
      SyntaxError::LabelledGeneratorOrAsync => {
        "Generator or async function cannot be labelled".into()
      }
      SyntaxError::LabelledFunctionInStrict => "Function cannot be labelled in strict mode".into(),
      ...
      SyntaxError::AwaitForStmt => "for await syntax is valid only for for-of statement".into(),
      ...
      SyntaxError::SpaceBetweenHashAndIdent => "Unexpected space between # and identifier".into(),
      ...
      SyntaxError::NullishCoalescingWithLogicalOp => {
        "Nullish coalescing operator(??) requires parens when mixing with logical operators".into()
      }

      SyntaxError::TS1056 => "jsc.target should be es5 or upper to use getter / setter".into(),
      ...
      SyntaxError::InvalidNameInUsingDecl => "Using declaration only allows identifiers".into(),
      SyntaxError::InitRequiredForUsingDecl => "Using declaration requires initializer".into(),
      SyntaxError::InvalidSuperCall => "Invalid `super()`".into(),
      SyntaxError::InvalidSuper => "Invalid access to super".into(),
      SyntaxError::InvalidSuperPrivateName => "Index super with private name is not allowed".into(),
      ...
      SyntaxError::GetterSetterCannotBeReadonly => "A getter or a setter cannot be readonly".into(),
      ...
      .into(),
      SyntaxError::TS1092 => "Type parameters cannot appear on a constructor declaration".into(),
      ...
      SyntaxError::TS1102 => "'delete' cannot be called on an identifier in strict mode".into(),
      ...
      SyntaxError::TS1171 => "A comma expression is not allowed in a computed property name".into(),
      ...
      SyntaxError::TS1183 => "An implementation cannot be declared in ambient contexts".into(),
      SyntaxError::TS1184 => "Modifiers cannot appear here".into(),
      SyntaxError::TS1185 => "Merge conflict marker encountered.".into(),
      SyntaxError::TS1093 => "Type annotation cannot appear on a constructor declaration".into(),
      ...
      SyntaxError::TS1244 => "Abstract methods can only appear within an abstract class.".into(),
