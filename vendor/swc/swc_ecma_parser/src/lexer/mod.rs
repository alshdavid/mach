...
use self::comments_buffer::CommentsBuffer;
pub use self::input::Input;
use self::state::State;
pub use self::state::TokenContext;
pub use self::state::TokenContexts;
use self::table::ByteHandler;
use self::table::BYTE_HANDLERS;
use self::util::*;
...
impl<'a> Lexer<'a> {
  ...
  fn read_ident_unknown(&mut self) -> LexResult<Token> {
    ...
    let (word, _) =
      self.read_word_as_str_with(|l, s, _, _| Word::Ident(IdentLike::Other(l.atoms.atom(s))))?;
  ...
  fn read_word_as_str_with<F, Ret>(
    &mut self,
    convert: F,
  ) -> LexResult<(Ret, bool)>
  where
  ...
  #[inline]
  pub fn set_next_regexp(
    &mut self,
    start: Option<BytePos>,
  ) {
