...
use crate::parser::class_and_fn::IsSimpleParameterList;
...
impl<I: Tokens> Parser<I> {
  ...
  pub(super) fn parse_primary_expr(&mut self) -> PResult<Box<Expr>> {
    ...
    if let Some(tok) = self.input.cur() {
      match tok {
        ...
        tok!("async") => {
          if peeked_is!(self, "function") && !self.input.has_linebreak_between_cur_and_peeked() {
            ...
            match bump!(self) {
              Token::Regex(exp, flags) => {
                ...
                let mut flags_count =
                  flags
                    .chars()
                    .fold(AHashMap::<char, usize>::default(), |mut map, flag| {
                      let key = match flag {
      ...
      || is!(self, IdentRef)
    {
        ...
        && is!(self, BindingIdent)
      {
        ...
        let params = vec![arg];
        expect!(self, "=>");
        let body = self.parse_fn_body(true, false, true, params.is_simple_parameter_list())?;
        ...
        let body = self.parse_fn_body(false, false, true, params.is_simple_parameter_list())?;
  ...
  fn parse_member_expr_or_new_expr_inner(
    &mut self,
    is_new_expr: bool,
  ) -> PResult<Box<Expr>> {
        ...
        if first {
          ...
          if is!(p, ')') {
            if is_dynamic_import && !p.input.syntax().import_attributes() {
              syntax_error!(p, span!(p, start), SyntaxError::TrailingCommaInsideImport)
            }
    ...
    if has_pattern || return_type.is_some() || is!(self, "=>") {
      ...
      if let BlockStmtOrExpr::BlockStmt(..) = &*arrow_expr.body {
        if let Ok(&Token::BinOp(..)) = cur!(self, false) {
          ...
          let errorred_expr = self.parse_bin_op_recursively(Box::new(arrow_expr.into()), 0)?;
  ...
  pub(super) fn parse_tpl_element(
    &mut self,
    is_tagged_tpl: bool,
  ) -> PResult<TplElement> {
    let start = cur_pos!(self);
        ...
        || eat!(self, '['))
    {
      ...
      return Ok((
        Box::new(match obj {
          ...
          Callee::Super(obj) => {
            if !self.ctx().allow_direct_super && !self.input.syntax().allow_super_outside_method() {
      ...
      return Ok((
        Box::new(match obj {
          ...
          Callee::Expr(obj) => {
            ...
            let expr =
              if unwrap_ts_non_null(&expr.obj).is_opt_chain() || question_dot_token.is_some() {
                Expr::OptChain(OptChainExpr {
                  span: span!(self, start),
                  optional: question_dot_token.is_some(),
                  base: Box::new(OptChainBase::Member(expr)),
        ...
        if is!(self, '?') {
            ...
            || peeked_is!(self, '=')
          {
            ...
            match *arg.expr {
              Expr::Ident(..) => {}
              _ => {
                syntax_error!(self, arg.span(), SyntaxError::TsBindingPatCannotBeOptional)
              }
            }
            true
...
impl<I: Tokens> Parser<I> {
  ...
  fn at_possible_async(
    &mut self,
    expr: &Expr,
  ) -> PResult<bool> {
    // TODO(kdy1): !this.state.containsEsc &&

    Ok(self.state.potential_arrow_start == Some(expr.span_lo()) && expr.is_ident_ref_to("async"))
  }
  ...
  pub(super) fn is_start_of_expr(&mut self) -> PResult<bool> {
    Ok(
      self.is_start_of_left_hand_side_expr()?
        || is_one_of!(
          self, '+', '-', '~', '!', "delete", "typeof", "void", "++", "--", '<', "await", "yield"
        )
