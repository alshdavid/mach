...
use crate::parser::expr::AssignTargetOrSpread;
use crate::token::IdentLike;
...
impl<I: Tokens> Parser<I> {
  ...
  #[allow(dead_code)]
  pub(super) fn parse_setter_param(
    &mut self,
    key_span: Span,
  ) -> PResult<Param> {
...
impl<I: Tokens> Parser<I> {
    ...
    match *expr {
      ...
      Expr::Assign(
        ...
        Ok(Pat::Assign(AssignPat {
          span,
          left: match left {
            AssignTarget::Simple(left) => Box::new(self.reparse_expr_as_pat(pat_ty, left.into())?),
            ...
            .map(|(idx, prop)| {
              ...
              match prop {
                PropOrSpread::Prop(prop) => match *prop {
                  ...
                  Prop::KeyValue(kv_prop) => Ok(ObjectPatProp::KeyValue(KeyValuePatProp {
                    key: kv_prop.key,
                    value: Box::new(self.reparse_expr_as_pat(pat_ty.element(), kv_prop.value)?),
                  })),
                  Prop::Assign(assign_prop) => Ok(ObjectPatProp::Assign(AssignPatProp {
                    span,
        ...
        if count_of_trailing_comma == 0 {
          ...
          let last = match expr {
            ...
            Some(ExprOrSpread {
              ...
              self
                .reparse_expr_as_pat(pat_ty.element(), expr)
                .map(|pat| {
                  Pat::Rest(RestPat {
                    span: expr_span,
                    dot3_token,
  ...
  /// eval
  fn pat_is_valid_argument_in_strict(
    &self,
    pat: &Pat,
  ) {
    match pat {
          ...
          match prop {
            ...
            ObjectPatProp::Assign(AssignPatProp { key, .. }) => {
              if key.is_reserved_in_strict_bind() {
                self.emit_strict_mode_err(key.span, SyntaxError::EvalAndArgumentsInStrict)
              }
            }
          }
        }
