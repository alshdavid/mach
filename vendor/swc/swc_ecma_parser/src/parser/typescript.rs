...
impl<I: Tokens> Parser<I> {
  ...
  pub(super) fn parse_ts_type_or_type_predicate_ann(
    ...
    self.in_type().parse_with(|p| {
      ...
      let has_type_pred_is =
        is!(p, IdentRef) && peeked_is!(p, "is") && !p.input.has_linebreak_between_cur_and_peeked();
  ...
  fn parse_ts_module_ref(&mut self) -> PResult<TsModuleRef> {
    ...
    if self.is_ts_external_module_ref()? {
      ...
      self
        .parse_ts_entity_name(/* allow_reserved_words */ false)
        .map(From::from)
    }
  }
  ...
  pub(super) fn ts_look_ahead<T, F>(
    &mut self,
    op: F,
  ) -> PResult<T>
  where
  ...
  fn parse_ts_type_member(&mut self) -> PResult<TsTypeElement> {
    ...
    self
      .parse_ts_property_or_method_signature(start, readonly)
      .map(|e| match e {
        Either::Left(e) => e.into(),
        Either::Right(e) => e.into(),
      })
  ...
  fn parse_ts_object_type_members(&mut self) -> PResult<Vec<TsTypeElement>> {
    ...
    expect!(self, '{');
    let members = self.parse_ts_list(ParsingContext::TypeMembers, |p| p.parse_ts_type_member())?;
    expect!(self, '}');
    Ok(members)
  }
  ...
  fn parse_ts_array_type_or_higher(
    &mut self,
    readonly: bool,
  ) -> PResult<Box<TsType>> {
...
mod tests {
  ...
  fn issue_708_1() {
    ...
    let expected = Module {
      ...
      shebang: None,
      body: {
        let first = ModuleItem::Stmt(Stmt::Decl(Decl::TsTypeAlias(Box::new(TsTypeAliasDecl {
          span: DUMMY_SP,
          declare: false,
