...
impl<I: Tokens> Parser<I> {
  ...
  pub(super) fn parse_jsx_opening_element_at(
    ...
    self
      .parse_jsx_opening_element_after_name(start, name)
      .map(Either::Right)
  }
          ...
          match *cur!(p, true)? {
            Token::JSXTagStart => {
              ...
              if peeked_is!(p, '/') {
                ...
                closing_element = p.parse_jsx_closing_element_at(start).map(Some)?;
                break 'contents;
              }
            ...
            Token::JSXText { .. } => children.push(p.parse_jsx_text().map(JSXElementChild::from)?),
            tok!('{') => {
              let start = cur_pos!(p);
              if peeked_is!(p, "...") {
                children.push(p.parse_jsx_spread_child().map(JSXElementChild::from)?);
              } else {
                children.push(
