...
impl<'a> SuperFieldAccessFolder<'a> {
  ...
  /// ```
  fn visit_mut_super_member_call(
    &mut self,
    n: &mut Expr,
  ) {
  ...
  fn visit_mut_super_member_update(
    &mut self,
    n: &mut Expr,
  ) {
      ...
      MemberExpr {
        ...
        obj: Box::new({
          let name = self
            .super_class
            .clone()
            .unwrap_or_else(|| quote_ident!(if self.is_static { "Function" } else { "Object" }));
