  ...
  fn fold_module_decl(
    ...
    match decl {
      ModuleDecl::ExportDefaultDecl(ExportDefaultDecl {
        ...
        let decorate_call =
          Box::new(self.fold_class_inner(ident.unwrap_or_else(|| quote_ident!("_class")), class));
    ...
    macro_rules! fold_method {
      ($method:expr, $fn_name:expr, $key_prop_value:expr) => {{
        ...
        Some(
          ObjectLit {
            span: DUMMY_SP,
            props: iter::once(PropOrSpread::Prop(Box::new(Prop::KeyValue(KeyValueProp {
              key: PropName::Ident(quote_ident!("kind")),
              value: Box::new(Expr::Lit(Lit::Str(quote_str!(match method.kind {
      ...
      .filter_map(|member| {
        ...
        match member {
          ...
          ClassMember::PrivateMethod(method) => {
            let fn_name = Ident::new(format!("_{}", method.key.id.sym).into(), method.key.id.span);
