...
impl VisitMut for Metadata<'_> {
  fn visit_mut_class(
    ...
    {
      let dec =
        self.create_metadata_design_decorator("design:type", quote_ident!("Function").as_arg());
          ...
          elems: constructor
            ...
            .map(|v| match v {
              ...
              ParamOrTsParamProp::Param(p) => {
                Some(serialize_type(self.class_name, get_type_ann_of_pat(&p.pat)).as_arg())
              }
            })
            .collect(),
        }
      ...
      let dec = self.create_metadata_design_decorator(
        ...
        ArrayLit {
          ...
          elems: m
            ...
            .params
            .iter()
            .map(|v| Some(serialize_type(self.class_name, get_type_ann_of_pat(&v.pat)).as_arg()))
            .collect(),
        }
        .as_arg(),
      );
...
impl<'a> Metadata<'a> {
  ...
  fn create_metadata_design_decorator(
    &self,
    design: &str,
    type_arg: ExprOrSpread,
  ) -> Decorator {
    Decorator {
      span: DUMMY_SP,
      expr: Box::new(Expr::Call(CallExpr {
  ...
  fn serialize_type_node(
    ...
    match ty {
      ...
      TsType::TsUnionOrIntersectionType(ty) => match ty {
        TsUnionOrIntersectionType::TsUnionType(ty) => serialize_type_list(class_name, &ty.types),
