...
use swc_ecma_utils::IdentRenamer;
...
impl Decorator202203 {
  ...
  fn preserve_side_effect_of_decorator(
    &mut self,
    dec: Box<Expr>,
  ) -> Box<Expr> {
  ...
  fn ensure_identity_constructor<'a>(
    &mut self,
    c: &'a mut Class,
  ) -> &'a mut Constructor {
    let mut insert_index = 0;
  ...
  fn process_decorators_of_class_members(
    &mut self,
        ...
        ClassMember::AutoAccessor(mut accessor) => {
          ...
          let private_field = PrivateProp {
            ...
            value: if accessor.decorators.is_empty() {
              ...
              let init_proto = if self.state.is_init_proto_called || accessor.is_static {
                None
              } else {
          ...
          if !accessor.decorators.is_empty() {
            let decorators = self.preserve_side_effect_of_decorators(accessor.decorators.take());
            let dec = merge_decorators(decorators);
                    ...
                    getter_function = Box::new(Function {
                      ...
                      body: Some(BlockStmt {
                        ...
                        stmts: vec![Stmt::Return(ReturnStmt {
                          ...
                          arg: Some(Box::new(Expr::Call(CallExpr {
                            span: DUMMY_SP,
                            callee: getter_var.clone().unwrap().as_callee(),
                            args: vec![ThisExpr { span: DUMMY_SP }.as_arg()],
                            type_args: Default::default(),
                          }))),
                        })],
                    ...
                    setter_function = Box::new(Function {
                      ...
                      body: Some(BlockStmt {
                        ...
                        stmts: vec![Stmt::Expr(ExprStmt {
                          ...
                          expr: Box::new(Expr::Call(CallExpr {
                            span: DUMMY_SP,
                            callee: setter_var.clone().unwrap().as_callee(),
                            args: vec![ThisExpr { span: DUMMY_SP }.as_arg(), param.as_arg()],
                            type_args: Default::default(),
                          })),
                        })],
                      }),
