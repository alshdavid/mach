...
pub use self::output::NormalizedOutput;
pub use self::output::StdErr;
pub use self::output::StdOut;
...
impl Tester {
    ...
    F: FnOnce(Lrc<SourceMap>, Handler) -> Result<Ret, ()>,
  {
    ...
    let (handler, errors) = self::diag_errors::new_handler(self.cm.clone(), self.treat_err_as_bug);
    let result = swc_common::GLOBALS.set(&self.globals, || op(self.cm.clone(), handler));
...
pub fn print_left_right(
  ...
  fn print(t: &dyn Debug) -> String {
    ...
    let s = {
      static RE: Lazy<Regex> = Lazy::new(|| Regex::new("Span \\{[\\a-zA-Z0#:\\(\\)]*\\}").unwrap());
