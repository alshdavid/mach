            ...
            .any(|e| e.expr.may_have_side_effects(&self.expr_ctx))
        {
          ...
          *expr = Expr::Lit(Lit::Num(Number {
          ...
          if elems.len() > idx as _ && idx >= 0 {
            let after_has_side_effect = elems.iter().skip((idx + 1) as _).any(|elem| match elem {
          ...
          Known(BoolType) | Known(NullType) | Known(NumberType) | Known(UndefinedType) => {
            match expr {
              Expr::Bin(BinExpr {
                ...
                if let Known(v) = self.perform_arithmetic_op(op!(bin, "+"), left, right) {
                  self.changed = true;
                  let span = *span;
...
impl VisitMut for SimplifyExpr {
            ...
            PropOrSpread::Spread(SpreadElement {
              ...
              if let Expr::Object(obj) = &*expr {
                if obj.props.iter().any(|p| match p {
                  ...
                  ps.push(PropOrSpread::Spread(SpreadElement { dot3_token, expr }));
                  continue;
                }
              }
  ...
  fn visit_mut_export_default_expr(
    &mut self,
    expr: &mut ExportDefaultExpr,
  ) {
    fn is_paren_wrap_fn_or_class(
      expr: &mut Expr,
      visitor: &mut SimplifyExpr,
    ) -> bool {
      match &mut *expr {
        Expr::Fn(..) | Expr::Class(..) => {
