      ...
      if let Some((value_idx, vi)) = value_idx {
        ...
        let barrier_exists = (|| {
          for &blocker in self.scope.inline_barriers.borrow().iter() {
            if (value_idx <= blocker && blocker <= idx) || (idx <= blocker && blocker <= value_idx)
            {
              return true;
            }
          }
...
impl<'a> Scope<'a> {
  ...
  fn should_prevent_inline_because_of_scope(
    &self,
    id: &Id,
  ) -> bool {
  ...
  fn read_prevents_inlining(
...

    {
      ...
      while let Some(scope) = cur {
        ...
        match scope.kind {
          ScopeKind::Fn { .. } => {
            tracing::trace!("{}: variable access from a nested function detected", id.0);
            return true;
          }
  ...
  pub fn find_binding_from_current(
    &self,
    id: &Id,
  ) -> Option<&VarInfo> {
