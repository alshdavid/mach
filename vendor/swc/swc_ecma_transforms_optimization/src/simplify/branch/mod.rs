...
use swc_ecma_transforms_base::perf::ParallelExt;
...
impl VisitMut for Remover {
  ...
  fn visit_mut_object_pat(
    ...
    p.props.retain(|p| match p {
      ...
      {
        if cfg!(feature = "debug") {
          debug!("Dropping key-value pattern property because it's an empty object pattern");
        }
        false
      }
      ...
      {
        if cfg!(feature = "debug") {
          debug!("Dropping key-value pattern property because it's an empty array pattern");
        }
        false
      }
      _ => true,
      ...
      match stmt {
        Stmt::If(IfStmt {
          ...
          if let (p, Known(v)) = test.cast_to_bool(&self.expr_ctx) {
            ...
            if !p.is_pure() {
              if let Some(expr) = ignore_result(*test, true, &self.expr_ctx).map(Box::new) {
        ...
        Stmt::Switch(mut s) => {
          ...
          let is_matching_literal = match *s.discriminant {
            Expr::Lit(Lit::Str(..)) | Expr::Lit(Lit::Null(..)) | Expr::Lit(Lit::Num(..)) => true,
            ref e if e.is_nan() || e.is_global_ref_to(&self.expr_ctx, "undefined") => true,
            _ => false,
          };
            ...
            s.cases.iter().position(|case| {
              ...
              if let Some(ref test) = case.test {
                let v = match (&**test, &*s.discriminant) {
                  ...
                  _ => {
                    if !test.is_nan() && !test.is_global_ref_to(&self.expr_ctx, "undefined") {
          ...
          if let Some(i) = selected {
            if !has_conditional_stopper(&s.cases[i].cons) {
              let mut exprs = vec![];
              exprs.extend(ignore_result(*s.discriminant, true, &self.expr_ctx).map(Box::new));
            ...
            s.cases = s.cases.move_flat_map(|case| {
              ...
              let res = match case.test {
                Some(e)
                  if matches!(
                    &*e,
                    Expr::Lit(Lit::Num(..)) | Expr::Lit(Lit::Str(..)) | Expr::Lit(Lit::Null(..))
                  ) =>
                {
                  case
                    .cons
              ...
              && !has_conditional_stopper(&s.cases.last().unwrap().cons)
            {
              ...
              exprs.extend(
                s.cases
                  ...
                  .filter_map(|e| ignore_result(*e, true, &self.expr_ctx).map(Box::new)),
              );
          ...
          if is_matching_literal
            && s.cases.iter().all(|case| match &case.test {
              Some(e)
                if matches!(
                  &**e,
                  Expr::Lit(Lit::Str(..)) | Expr::Lit(Lit::Null(..)) | Expr::Lit(Lit::Num(..))
                ) =>
              {
                true
              }
      ...
      let stmt_like = match stmt_like.try_into_stmt() {
        Ok(stmt) => {
          let stmt = match stmt {
            ...
            // Control flow
            Stmt::Throw(..) | Stmt::Return { .. } | Stmt::Continue { .. } | Stmt::Break { .. } => {
  ...
  match e {
    ...
    Expr::Array(ArrayLit { span, elems, .. }) => {
      ...
      let elems = elems.move_flat_map(|v| match v {
        ...
        None => None,
        Some(ExprOrSpread { spread: None, expr }) => ignore_result(*expr, true, ctx).map(|expr| {
          Some(ExprOrSpread {
            spread: None,
            expr: Box::new(expr),
          })
    ...
    Expr::TaggedTpl(TaggedTpl { span, tag, tpl, .. }) if tag.is_pure_callee(ctx) => ignore_result(
...
fn is_block_scoped_stuff(s: &Stmt) -> bool {
  match s {
    Stmt::Decl(Decl::Var(v)) if v.kind == VarDeclKind::Const || v.kind == VarDeclKind::Let => true,
