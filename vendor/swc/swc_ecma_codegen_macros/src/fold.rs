...
impl Fold for InjectSelf {
  fn fold_signature(
    ...
    self.parser = i.inputs.first().cloned().and_then(|arg| match arg {
      ...
      | FnArg::Receiver(Receiver { self_token, .. }) => Some(Ident::new("self", self_token.span())),
      _ => None,
    });
    i
  }
  ...
  fn fold_macro(
    ...
    match &*name {
      ...
      "println" | "print" | "format" | "assert" | "assert_eq" | "assert_ne" | "debug_assert"
      | "debug_assert_eq" | "debug_assert_ne" | "dbg" => {
      ...
      "keyword" | "emit" | "punct" | "semi" | "formatting_semi" | "space" | "formatting_space"
      | "operator" | "opt" | "opt_leading_space" | "srcmap" => {
