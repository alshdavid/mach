...
use self::hook::HookRegister;
use self::util::collect_ident_in_jsx;
use self::util::is_body_arrow_fn;
use self::util::is_import_or_require;
use self::util::make_assign_stmt;
  ...
  fn get_persistent_id_from_possible_hoc(
    ...
    match first_arg.as_mut() {
      Expr::Call(expr) => {
        ...
        if let Persist::Hoc(hoc) = self.get_persistent_id_from_possible_hoc(expr, reg, hook_reg) {
  ...
  fn visit_mut_module_items(
    ...
    for mut item in module_items.take() {
      let persistent_id = match &mut item {
        ...
        ModuleItem::ModuleDecl(ModuleDecl::ExportDefaultExpr(ExportDefaultExpr { expr, span })) => {
          if let Expr::Call(call) = expr.as_mut() {
            if let Persist::Hoc(Hoc { reg, hook, .. }) = self.get_persistent_id_from_possible_hoc(
              call,
              vec![(
                private_ident!("_c"),
