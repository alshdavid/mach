...
use crate::refresh::options::deserialize_refresh;
use crate::refresh::options::RefreshOptions;
            ...
            Some("@jsxRuntime") => match val {
              ...
              _ => {
                HANDLER.with(|handler| {
                  handler
                    .struct_span_err(cmt.span, "Runtime must be either `classic` or `automatic`.")
                    .emit()
                });
              }
            },
            ...
            Some("@jsx") => {
              if let Some(src) = val {
                if is_valid_for_pragma(src) {
                  // TODO: Optimize
                  let mut e =
                    (*parse_expr_for_jsx(cm, "module-jsx-pragma", src.to_string(), top_level_mark))
                      .clone();
                  respan(&mut e, cmt.span);
  ...
  C: Comments,
{
  ...
  fn jsx_frag_to_expr(
    ...
    let use_jsxs =
      count > 1 || (count == 1 && matches!(&el.children[0], JSXElementChild::JSXSpreadChild(..)));
    ...
    match self.runtime {
      Runtime::Automatic => {
        ...
        let args = if self.development {
          args
            .chain(once(undefined(DUMMY_SP).as_arg()))
