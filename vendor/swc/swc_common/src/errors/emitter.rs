...
use super::diagnostic::Message;
use super::snippet::Annotation;
use super::snippet::AnnotationType;
use super::snippet::Line;
use super::snippet::MultilineAnnotation;
use super::snippet::Style;
use super::snippet::StyledString;
use super::styled_buffer::StyledBuffer;
use super::CodeSuggestion;
use super::DiagnosticBuilder;
use super::DiagnosticId;
use super::Level;
use super::SourceMapperDyn;
use super::SubDiagnostic;
use crate::sync::Lrc;
...
impl EmitterWriter {
  ...
  fn preprocess_annotations(
    ...
    for item in multiline_annotations.clone() {
      ...
      for item in multiline_annotations.iter_mut() {
        ...
        if &ann != a && num_overlap(ann.line_start, ann.line_end, a.line_start, a.line_end, true) {
          a.increase_depth();
        } else {
          break;
        }
  ...
  fn msg_to_buffer(
    ...
    /// `style` otherwise
    fn style_or_override(
      style: Style,
      override_: Option<Style>,
    ) -> Style {
  ...
  fn emit_message_default(
    ...
    let (primary_lo, sm) =
      if let (Some(sm), Some(primary_span)) = (self.sm.as_ref(), msp.primary_span().as_ref()) {
        if !primary_span.is_dummy() {
          ...
          if line_idx < (annotated_file.lines.len() - 1) {
            ...
            if line_idx_delta > 2 {
              ...
              buffer.puts(
                last_buffer_line_num,
                0,
                &self.maybe_anonymized(annotated_file.lines[line_idx + 1].line_index - 1),
                Style::LineNumber,
              );
    ...
    match self.emit_message_default(span, message, code, level, max_line_num_len, false) {
      Ok(()) => {
        if !children.is_empty() {
          ...
          match emit_to_destination(&buffer.render(), level, &mut self.dst, self.short_message) {
            Ok(()) => (),
        ...
        if !self.short_message {
          ...
          for sugg in suggestions {
            if let Err(e) = self.emit_suggestion_default(sugg, Level::Help, max_line_num_len) {
              panic!("failed to emit error: {}", e)
            }
          }
        }
