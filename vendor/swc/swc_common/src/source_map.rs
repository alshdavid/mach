...
use crate::sync::Lrc;
use crate::sync::MappedLockGuard;
...
impl SourceMap {
  ...
  pub fn span_to_unmapped_path(
    &self,
    sp: Span,
  ) -> FileName {
    self
      .lookup_char_pos(sp.lo())
      .file
      .unmapped_path
      .clone()
  ...
  /// be returned.
  pub fn span_until_non_whitespace(
    &self,
    sp: Span,
  ) -> Span {
  ...
  {
    self
      .span_to_source(sp, |src, start_index, end_index| {
        let snippet = &src[start_index..end_index];
  ...
  /// span.
  fn find_width_of_character_at_span(
    &self,
    sp: Span,
    forwards: bool,
  ) -> u32 {
  ...
  pub fn span_to_char_offset(
    ...
    let start =
      span.lo.to_u32() - start_offset.to_u32() - self.calc_utf16_offset(file, span.lo, &mut state);
    let end =
      span.hi.to_u32() - start_offset.to_u32() - self.calc_utf16_offset(file, span.hi, &mut state);
  ...
  pub fn generate_fn_name_span(
    &self,
    span: Span,
  ) -> Option<Span> {
    let prev_span = self.span_extend_to_prev_str(span, "fn", true);
    self
      .span_to_snippet(prev_span)
      .map(|snippet| {
        let len = snippet
  ...
  pub fn build_source_map_with_config(
    ...
    for (pos, lc) in mappings.iter() {
      ...
      let linechpos = linebpos.to_u32() - self.calc_utf16_offset(f, linebpos, &mut line_state);
