...
impl VisitMut for Fixer<'_> {
    ...
    match n {
      ...
      AssignTarget::Pat(b) => {
        if let AssignTargetPat::Invalid(_) = b {
          *n = AssignTarget::Simple(SimpleAssignTarget::Invalid(Invalid { span: DUMMY_SP }));
        }
      }
    }
  }
  ...
  fn visit_mut_bin_expr(
    ...
    match &mut *expr.right {
      Expr::Assign(..) | Expr::Seq(..) | Expr::Yield(..) | Expr::Cond(..) | Expr::Arrow(..) => {
        self.wrap(&mut expr.right);
      }
  ...
  fn visit_mut_export_default_expr(
    ...
    match &mut *node.expr {
      ...
      Expr::Fn(FnExpr { ident: Some(_), .. }) | Expr::Class(ClassExpr { ident: Some(_), .. }) => {
  ...
  fn visit_mut_unary_expr(
    ...
    match &*n.arg {
      Expr::Bin(BinExpr {
        ...
        && match (&**left, &**right) {
          (Expr::Lit(Lit::Num(l)), Expr::Lit(Lit::Num(..))) => !l.value.is_sign_negative(),
          _ => false,
        } => {}
...
impl Fixer<'_> {
  fn wrap_with_paren_if_required(
    &mut self,
    e: &mut Expr,
  ) {
    ...
    match e {
            ...
            match &mut **expr {
              Expr::Seq(SeqExpr { exprs, .. }) => {
                ...
                if !is_last {
                  buf.extend(
                    exprs
                      .into_iter()
                      .filter_map(|expr| ignore_return_value(expr, &mut has_padding_value)),
                  );
                } else {
                  let exprs_len = exprs.len();
                  for (i, expr) in exprs.into_iter().enumerate() {
                    ...
                    if is_last {
                      ...
                      buf.extend(ignore_return_value(expr, &mut has_padding_value));
                    }
                  }
                }
              }
              _ => {
                if is_last {
                  ...
                  buf.extend(ignore_return_value(expr.take(), &mut has_padding_value));
                }
              }
            }
          }
      ...
      Expr::Cond(expr) => {
        match &mut *expr.test {
          Expr::Seq(..) | Expr::Assign(..) | Expr::Cond(..) | Expr::Arrow(..) | Expr::Yield(..) => {
