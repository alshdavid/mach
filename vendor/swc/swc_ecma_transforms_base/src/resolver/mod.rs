...
impl<'a> Resolver<'a> {
  ...
  fn visit_mut_stmt_within_child_scope(
    &mut self,
    s: &mut Stmt,
  ) {
...
impl Hoister<'_, '_> {
  fn add_pat_id(
    ...
    if self.in_catch_body {
      ...
      if self.catch_param_decls.contains(&id.sym) && !self.excluded_from_catch.contains(&id.sym) {
        return;
      }
    }
