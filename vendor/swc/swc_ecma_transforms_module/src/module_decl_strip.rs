  ...
  fn from(e: &ExportSpecifier) -> Self {
    match e {
      ...
      ExportSpecifier::Named(ExportNamedSpecifier {
        is_type_only: false,
        orig:
          ModuleExportName::Ident(Ident { sym: s, .. }) | ModuleExportName::Str(Str { value: s, .. }),
        ..
...
impl Extend<LinkSpecifier> for LinkItem {
  fn extend<T: IntoIterator<Item = LinkSpecifier>>(
    &mut self,
    iter: T,
  ) {
    ...
    self.into_iter().for_each(|s| match s {
      ...
      LinkSpecifier::ExportDefaultAs(_, key, span) => {
        *ref_to_mod_ident = true;
        ...
        export_obj_prop_list.push((key.clone(), ExportItem::new(span, quote_ident!(span, key))));
      }
