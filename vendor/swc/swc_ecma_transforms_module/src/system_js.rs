...
use crate::top_level_this::top_level_this;
use crate::util::local_name_for_src;
use crate::util::use_strict;
...
impl SystemJs {
                ...
                if to == *k {
                  for _ in v.iter() {
                    exprs.push(Box::new(Expr::Call(self.export_call(
                      to.0.clone(),
                      DUMMY_SP,
                      Ident::new(to.0.clone(), DUMMY_SP.with_ctxt(to.1)).into(),
                    ))));
                  }
                  break;
                }
  ...
  fn build_module_item_export_all(
    &mut self,
    mut meta: ModuleItemMeta,
  ) -> Vec<Stmt> {
    if !meta.has_export_all {
      meta
        .setter_fn_stmts
        .append(&mut self.build_export_call(&mut meta.export_names, &mut meta.export_values));
    } else {
  ...
  fn add_module_item_meta(
    &mut self,
    mut m: ModuleItemMeta,
  ) {
    match self
  ...
  fn hoist_for_var_decl(
    &mut self,
    var_decl_or_pat: ForHead,
  ) -> ForHead {
...
impl Fold for SystemJs {
  ...
  fn fold_module(
      ...
      match item {
        ModuleItem::ModuleDecl(decl) => match decl {
          ModuleDecl::Import(import) => {
            let src = match &self.resolver {
              Resolver::Real { resolver, base } => resolver
                ...
                .with_context(|| format!("failed to resolve import `{}`", import.src.value))
                .unwrap(),
                  ...
                  setter_fn_stmts.push(
                    AssignExpr {
                      ...
                      right: Box::new(Expr::Member(MemberExpr {
                        span: DUMMY_SP,
                        obj: Box::new(Expr::Ident(quote_ident!(source_alias.clone()))),
                ...
                ImportSpecifier::Namespace(specifier) => {
                  ...
                  setter_fn_stmts.push(
                    AssignExpr {
                      ...
                      op: op!("="),
                      left: specifier.local.into(),
                      right: Box::new(Expr::Ident(quote_ident!(source_alias.clone()))),
                    }
                    .into_stmt(),
                  );
                }
              ...
              for specifier in decl.specifiers {
                ...
                match specifier {
                  ...
                  ExportSpecifier::Namespace(specifier) => {
                    export_names.push(get_module_export_name(&specifier.name).0);
                    export_values.push(Box::new(Expr::Ident(quote_ident!(source_alias.clone()))));
                  }
                }
              ...
              for specifier in decl.specifiers {
                if let ExportSpecifier::Named(specifier) = specifier {
                  ...
                  if self.root_fn_decl_idents.iter().any(|i| i.sym == id.0) {
                    ...
                    self
                      .export_values
                      .push(Box::new(get_module_export_expr(&specifier.orig)));
                  }
          ...
          ModuleDecl::ExportDecl(decl) => {
            match decl.decl {
              ...
              Decl::Fn(fn_decl) => {
                ...
                self.add_export_name(fn_decl.ident.to_id(), fn_decl.ident.sym.clone());
              ...
              Decl::Var(var_decl) => {
                ...
                for var_declarator in var_decl.decls {
                  ...
                  var_declarator.visit_with(&mut VarCollector { to: &mut tos });
                  for to in tos {
                    let ident = Ident::new(to.0.clone(), DUMMY_SP.with_ctxt(to.1));
          ...
          ModuleDecl::ExportDefaultExpr(expr) => {
            execute_stmts.push(
              self
                .export_call("default".into(), expr.span, *expr.expr)
                .into_stmt(),
            );
          }
    ...
    if !self.export_names.is_empty() {
      ...
      before_body_stmts.append(&mut self.build_export_call(&mut export_names, &mut export_values));
    }
    ...
    let module_item_meta_list: Vec<ModuleItemMeta> = self.module_item_meta_list.drain(..).collect();
