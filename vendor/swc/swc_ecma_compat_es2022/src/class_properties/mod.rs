...
use self::class_name_tdz::ClassNameTdzFolder;
use self::member_init::MemberInit;
use self::member_init::MemberInitRecord;
use self::member_init::PrivAccessor;
use self::member_init::PrivMethod;
use self::member_init::PrivProp;
use self::member_init::PubProp;
use self::private_field::dup_private_method;
use self::private_field::visit_private_in_expr;
use self::private_field::BrandCheckHandler;
use self::private_field::Private;
use self::private_field::PrivateAccessVisitor;
use self::private_field::PrivateKind;
use self::private_field::PrivateRecord;
use self::this_in_static::NewTargetInProp;
use self::this_in_static::ThisInStaticFolder;
use self::used_name::UsedNameCollector;
...
#[swc_trace]
impl ClassExtra {
  fn prepend_with<T: StmtLike + From<Stmt>>(
    self,
    stmts: &mut Vec<T>,
  ) {
  ...
  fn merge_with<T: StmtLike + From<Stmt>>(
    self,
    stmts: &mut Vec<T>,
    class: T,
  ) {
          ...
          Ok(mut decl) => {
            match decl {
              ModuleDecl::ExportDefaultDecl(ExportDefaultDecl {
                ...
                buf.push(
                  match T::try_from_module_decl(ModuleDecl::ExportNamed(NamedExport {
                    span,
          ...
          match member {
            ClassMember::PrivateMethod(method) => {
              if let Some(kind) = private_map.get_mut(&method.key.id.sym) {
                if dup_private_method(kind, method) {
                  let error = format!("duplicate private name #{}.", method.key.id.sym);
                  HANDLER
                    .with(|handler| handler.struct_span_err(method.key.id.span, &error).emit());
                } else {
                  match method.kind {
            ...
            ClassMember::PrivateProp(prop) => {
              if private_map.get(&prop.key.id.sym).is_some() {
                ...
                HANDLER.with(|handler| handler.struct_span_err(prop.key.id.span, &error).emit());
              } else {
                private_map.insert(
        ...
        ClassMember::PrivateMethod(mut method) => {
          ...
          let extra_collect = match (method.kind, is_static) {
            (MethodKind::Getter | MethodKind::Setter, false) => {
              ...
              let inserted = constructor_inits.push(MemberInit::PrivAccessor(PrivAccessor {
                span: prop_span,
            ...
            (MethodKind::Getter | MethodKind::Setter, true) => {
              let is_getter = method.kind == MethodKind::Getter;
              let inserted = extra_inits.push(MemberInit::PrivAccessor(PrivAccessor {
                span: prop_span,
                name: weak_coll_var.clone(),
                getter: if is_getter {
                  Some(fn_name.clone())
