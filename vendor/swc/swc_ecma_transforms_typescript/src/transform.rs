...
use swc_ecma_utils::QueryRef;
use swc_ecma_utils::RefRewriter;
...
use crate::config::TsImportExportAssignConfig;
use crate::ts_enum::EnumValueComputer;
use crate::ts_enum::InlineEnum;
use crate::ts_enum::TsEnumRecord;
use crate::ts_enum::TsEnumRecordKey;
use crate::ts_enum::TsEnumRecordValue;
use crate::utils::assign_value_to_this_private_prop;
use crate::utils::assign_value_to_this_prop;
use crate::utils::AsCollapsibleDecl;
use crate::utils::AsEnumOrModule;
use crate::utils::Factory;
use crate::utils::VecStmtLike;
...
impl Transform {
  fn visit_mut_stmts_for_enum_or_module(
    &mut self,
    n: &mut Vec<Stmt>,
  ) {
  ...
  fn visit_mut_module_items_for_enum_or_module(
    &mut self,
    n: &mut Vec<ModuleItem>,
  ) {
    let found = (**n)
...
impl Transform {
  ...
  fn transform_ts_namespace_body(
    ...
    let init_arg = Self::get_enum_or_module_init_arg(&Some(id), local_name.clone(), true).into();
          ...
          match decl.module_ref {
            TsModuleRef::TsEntityName(ts_entity_name) => {
              ...
              let stmt = if decl.is_export {
                ...
                // const foo = bar.baz
                let mut var_decl = init.into_var_decl(VarDeclKind::Const, decl.id.clone().into());
...
impl Transform {
  fn transform_ts_enum(
    ...
    for m in members {
      ...
      let value =
        Self::transform_ts_enum_member(m, &id.to_id(), &default_init, record, self.top_level_mark);
...
impl Transform {
  fn transform_export_decl_in_ts_module_block(
    ...
    match decl {
      ...
      Decl::Var(var_decl) => [
        Self::transform_export_var_decl_in_ts_module_block(*var_decl, id, span, mutable_export_ids),
        None,
      ],
      _ => unreachable!(),
    }
  ...
  fn transform_export_var_decl_in_ts_module_block(
    ...
    let mut expr_list: Vec<Box<Expr>> = var_decl.decls.into_iter().filter_map(|d| d.init).collect();
...
impl Transform {
  ...
  fn visit_mut_for_ts_import_export(
    &mut self,
    n: &mut Module,
  ) {
    let n = &mut n.body;
        ...
        ModuleItem::ModuleDecl(ModuleDecl::TsImportEquals(decl)) if !decl.is_type_only => {
          ...
          match &mut decl.module_ref {
            ...
            TsModuleRef::TsEntityName(ts_entity_name) => {
              ...
              let mut var_decl = init.into_var_decl(VarDeclKind::Const, decl.id.take().into());
