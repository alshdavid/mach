...
use crate::ImportsNotUsedAsValues;
...
fn get_module_ident(ts_entity_name: &TsEntityName) -> &Ident {
  match ts_entity_name {
    TsEntityName::TsQualifiedName(ts_qualified_name) => get_module_ident(&ts_qualified_name.left),
  ...
  fn visit_mut_module_items(
    ...
    n.retain_mut(|module_item| match module_item {
        ...
        if !specifiers.is_empty() =>
      {
        ...
        self.import_not_used_as_values == ImportsNotUsedAsValues::Preserve || !specifiers.is_empty()
      }
      ...
      ModuleItem::ModuleDecl(ModuleDecl::ExportNamed(NamedExport { ref type_only, .. })) => {
        !type_only
      }
      ...
      ModuleItem::Stmt(Stmt::Decl(Decl::TsModule(ref ts_module))) if ts_module.body.is_some() => {
