  ...
  fn super_update(
    ...
    if let Some(callee) = self.super_update.ident.get(prop_name) {
      ...
      self
        ...
        .or_insert_with(|| private_ident!(prop_span, format!("_superprop_get_{}", prop_name)));

      self
        .super_set
        .ident
        .entry(prop_name.clone())
        .or_insert_with(|| private_ident!(prop_span, format!("_superprop_set_{}", prop_name)));
    ...
    match e {
      Expr::Ident(Ident { span, sym, .. })
        if !self.arguments_disabled
          && *sym == "arguments"
          && (span.ctxt == self.unresolved_ctxt || span.ctxt == SyntaxContext::empty()) =>
      {
        let arguments = self
          .args
