...
use swc_ecma_visit::visit_obj_and_computed;
...
#[allow(deprecated)]
pub use self::factory::FunctionFactory;
#[allow(deprecated)]
pub use self::factory::IntoIndirectCall;
#[allow(deprecated)]
...
pub trait ExprExt {
  ...
  /// Returns `true` if `id` references a global object.
  fn is_one_of_global_ref_to(
    &self,
    ctx: &ExprCtx,
    ids: &[&str],
  ) -> bool {
    match self.as_expr() {
      Expr::Ident(i) => i.span.ctxt == ctx.unresolved_ctxt && ids.iter().any(|id| i.sym == *id),
      _ => false,
    }
  }
          ...
          let e = match *elem {
            Some(ref elem) => {
              ...
              match &**expr {
                ...
                Expr::Ident(Ident { sym: undefined, .. }) if &**undefined == "undefined" => {
                  Cow::Borrowed("")
                }
        ...
        obj.is_global_ref_to(ctx, "Math")
          || match &**obj {
            ...
            Expr::Lit(Lit::Str(..)) => match &*prop.sym {
              ...
              | "lastIndexOf" | "localeCompare" | "slice" | "split" | "startsWith" | "substr"
  ...
  fn may_have_side_effects(
    ...
    match self.as_expr() {
      ...
      Expr::Lit(..) | Expr::This(..) | Expr::PrivateName(..) | Expr::TsConstAssertion(..) => false,
            ...
            let can_have_side_effect = |prop: &PropOrSpread| match prop {
              ...
              PropOrSpread::Prop(prop) => match prop.as_ref() {
                ...
                | Prop::KeyValue(KeyValueProp {
                  key: PropName::Ident(Ident { sym, .. }) | PropName::Str(Str { value: sym, .. }),
                  ..
                }) => &**sym == "__proto__",
                Prop::KeyValue(KeyValueProp {
...
pub fn is_simple_pure_member_expr(
  m: &MemberExpr,
  pure_getters: bool,
) -> bool {
  match &m.prop {
...
/// Used to determine super_class_ident
pub fn alias_ident_for_simple_assign_tatget(
  expr: &SimpleAssignTarget,
  default: &str,
) -> Ident {
  let span = expr.span().apply_mark(Mark::fresh(Mark::root()));
...
pub fn prepend_stmt<T: StmtLike>(
  stmts: &mut Vec<T>,
  stmt: T,
) {
  let idx = stmts
...
impl ExprCtx {
  ...
  /// c())
  #[allow(clippy::vec_box)]
  pub fn extract_side_effects_to(
    &self,
    to: &mut Vec<Box<Expr>>,
    expr: Expr,
  ) {
    match expr {
      Expr::Lit(..) | Expr::This(..) | Expr::Fn(..) | Expr::Arrow(..) | Expr::PrivateName(..) => {}
      ...
      | Expr::TsSatisfies(TsSatisfiesExpr { expr, .. }) => self.extract_side_effects_to(to, *expr),
