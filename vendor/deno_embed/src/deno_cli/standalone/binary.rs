...
impl<'a> DenoCompileBinaryWriter<'a> {
  ...
  async fn get_base_binary(
    ...
    let binary_path_suffix = if crate::deno_cli::version::is_canary() {
      format!("canary/{}/{}", crate::deno_cli::version::GIT_COMMIT_HASH, binary_name)
    } else {
  ...
  fn build_vfs(&self) -> Result<VfsBuilder, AnyError> {
    fn maybe_warn_different_system(system_info: &NpmSystemInfo) {
      if system_info != &NpmSystemInfo::default() {
        log::warn!("{} The node_modules directory may be incompatible with the target system.", crate::deno_cli::colors::yellow("Warning"));
      }
    }
