...
macro_rules! lsp_log {
  ($($arg:tt)+) => (
    let lsp_log_level = $crate::deno_cli::lsp::logging::lsp_log_level();
    if lsp_log_level == log::Level::Debug {
      $crate::deno_cli::lsp::logging::lsp_debug!($($arg)+)
    } else {
      let s = std::format!($($arg)+);
      $crate::deno_cli::lsp::logging::write_line_to_log_file(&s);
...
macro_rules! lsp_warn {
  ($($arg:tt)+) => (
    {
      let lsp_log_level = $crate::deno_cli::lsp::logging::lsp_warn_level();
      if lsp_log_level == log::Level::Debug {
        $crate::deno_cli::lsp::logging::lsp_debug!($($arg)+)
      } else {
        let s = std::format!($($arg)+);
  ...
  ($($arg:tt)+) => (
    {
      let s = std::format!($($arg)+);
      $crate::deno_cli::lsp::logging::write_line_to_log_file(&s);
      if $crate::deno_cli::lsp::logging::lsp_debug_enabled() {
        log::debug!("{}", s)
      }
    }
  )
