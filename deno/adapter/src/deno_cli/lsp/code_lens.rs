...
impl DenoTestCollector {
  ...
  fn add_code_lenses<N: AsRef<str>>(
    &mut self,
    name: N,
    range: &SourceRange,
  ) {
        ...
        ast::Expr::Member(member_expr) => {
          if let ast::MemberProp::Ident(ns_prop_ident) = &member_expr.prop {
            ...
            if matches!(ns_prop_ident.sym.as_str(), "ignore" | "only") {
              let ast::Expr::Member(member_expr_) = member_expr.obj.as_ref() else {
                return;
              };
              member_expr = member_expr_;
              let ast::MemberProp::Ident(ns_prop_ident_) = &member_expr.prop else {
                return;
              };
