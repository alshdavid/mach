...
use crate::deno_cli::tools::lint::create_linter;
...
impl<'a> TsResponseImportMapper<'a> {
  ...
  pub fn check_specifier(
    ...
    if let Some(npm_resolver) = self.npm_resolver.as_ref().and_then(|r| r.as_managed()) {
      if npm_resolver.in_npm_package(specifier) {
        if let Ok(Some(pkg_id)) = npm_resolver.resolve_pkg_id_from_specifier(specifier) {
...
fn try_reverse_map_package_json_exports(
  ...
  let result = try_reverse_map_package_json_exports_inner(root_path, target_path, exports)?;
  if result.is_empty() {
    None
  } else {
    Some(result)
  ...
  if action.fix_name == "import" {
    ...
    if let Some(captures) = IMPORT_SPECIFIER_RE.captures(&text_change.new_text) {
      let specifier = captures
        ...
        .as_str();
      if let Some(new_specifier) = import_mapper.check_unresolved_specifier(specifier, referrer) {
