...
use crate::deno_cli::graph_util::enhanced_resolution_error_message;
use crate::deno_cli::lsp::lsp_custom::DiagnosticBatchNotificationParams;
use crate::deno_cli::resolver::SloppyImportsResolution;
use crate::deno_cli::resolver::SloppyImportsResolver;
use crate::deno_cli::tools::lint::get_configured_rules;
...
struct DiagnosticsPublisher {
  ...
  diagnostics_by_specifier: Mutex<HashMap<ModuleSpecifier, DiagnosticsBySource>>,
}
...
impl DiagnosticsPublisher {
  ...
  pub async fn publish(
    ...
    let mut diagnostics_by_specifier = self.diagnostics_by_specifier.lock().await;
    ...
    for (specifier, diagnostics_by_source) in diagnostics_by_specifier.iter_mut() {
...
pub fn should_send_diagnostic_batch_index_notifications() -> bool {
  crate::deno_cli::args::has_flag_env_var("DENO_DONT_USE_INTERNAL_LSP_DIAGNOSTIC_SYNC_FLAG")
}
...
impl DiagnosticsState {
  fn update(
    ...
    for diagnostic in diagnostics {
      if diagnostic.code == Some(lsp::NumberOrString::String("no-cache".to_string()))
        || diagnostic.code == Some(lsp::NumberOrString::String("no-cache-jsr".to_string()))
        || diagnostic.code == Some(lsp::NumberOrString::String("no-cache-npm".to_string()))
      {
  ...
  pub fn has_no_cache_diagnostics(
    &self,
    specifier: &ModuleSpecifier,
  ) -> bool {
    self
            ...
            Some(message) => {
              ...
              ts_handle = Some(spawn({
                ...
                async move {
                  ...
                  let diagnostics =
                    generate_ts_diagnostics(snapshot.clone(), &config, &ts_server, token.clone())
                      .await
                      .map_err(|err| {
                        if !token.is_cancelled() {
                  ...
                  if !token.is_cancelled() {
                    ...
                    messages_len = diagnostics_publisher
                      .publish(DiagnosticSource::Ts, diagnostics, &url_map, &token)
                      .await;
                  ...
                  if let Some(batch_index) = batch_index {
                    diagnostics_publisher
                      .client
                      .send_diagnostic_batch_notification(DiagnosticBatchNotificationParams {
                        batch_index,
                        messages_len,
                      });
                  }
              ...
              deps_handle = Some(spawn({
                ...
                async move {
                  ...
                  if !token.is_cancelled() {
                    messages_len = diagnostics_publisher
                      .publish(DiagnosticSource::Deno, diagnostics, &url_map, &token)
                      .await;
              ...
              lint_handle = Some(spawn({
                ...
                async move {
                  ...
                  let diagnostics = spawn_blocking({
                    let token = token.clone();
                    move || generate_lint_diagnostics(&snapshot, &config, &lint_options, token)
                  })
                  .await
                  .unwrap();
                  ...
                  if !token.is_cancelled() {
                    messages_len = diagnostics_publisher
                      .publish(DiagnosticSource::Lint, diagnostics, &url_map, &token)
                      .await;
...
impl<'a> From<&'a crate::deno_cli::tsc::DiagnosticCategory> for lsp::DiagnosticSeverity {
  fn from(category: &'a crate::deno_cli::tsc::DiagnosticCategory) -> Self {
    match category {
      crate::deno_cli::tsc::DiagnosticCategory::Error => lsp::DiagnosticSeverity::ERROR,
      crate::deno_cli::tsc::DiagnosticCategory::Warning => lsp::DiagnosticSeverity::WARNING,
      crate::deno_cli::tsc::DiagnosticCategory::Suggestion => lsp::DiagnosticSeverity::HINT,
      crate::deno_cli::tsc::DiagnosticCategory::Message => lsp::DiagnosticSeverity::INFORMATION,
    }
  }
}

impl<'a> From<&'a crate::deno_cli::tsc::Position> for lsp::Position {
  fn from(pos: &'a crate::deno_cli::tsc::Position) -> Self {
    Self {
...
fn get_diagnostic_message(diagnostic: &crate::deno_cli::tsc::Diagnostic) -> String {
...
fn to_lsp_range(
  start: &crate::deno_cli::tsc::Position,
  end: &crate::deno_cli::tsc::Position,
) -> lsp::Range {
  lsp::Range {
    start: start.into(),
    end: end.into(),
...
fn to_lsp_related_information(
  related_information: &Option<Vec<crate::deno_cli::tsc::Diagnostic>>
...
fn ts_json_to_diagnostics(
  diagnostics: Vec<crate::deno_cli::tsc::Diagnostic>
) -> Vec<lsp::Diagnostic> {
  diagnostics
    .iter()
    .filter_map(|d| {
...
fn generate_lint_diagnostics(
  ...
  let lint_rules =
    get_configured_rules(lint_options.rules.clone(), config.config_file.as_ref()).rules;
  let mut diagnostics_vec = Vec::new();
  for document in documents {
    let settings = config.workspace_settings_for_specifier(document.specifier());
    if !settings.lint {
      continue;
    }
...
impl DenoDiagnostic {
    ...
    if let Some(lsp::NumberOrString::String(code)) = &diagnostic.code {
      let code_action = match code.as_str() {
        ...
        "no-cache" | "no-cache-jsr" | "no-cache-npm" => {
          ...
          lsp::CodeAction {
            title: format!("Cache \"{}\" and its dependencies.", data.specifier),
            kind: Some(lsp::CodeActionKind::QUICKFIX),
            diagnostics: Some(vec![diagnostic.clone()]),
  ...
  pub fn to_lsp_diagnostic(
    ...
    fn no_local_message(
      ...
      let mut message = format!("Unable to load a local module: {}\n", specifier);
      if let Some(additional_message) = sloppy_resolution.as_suggestion_message() {
      ...
      if let Some(doc) = snapshot.documents.get(specifier) {
          ...
          .and_then(|n| n.npm_resolver.as_managed())
        {
          ...
          if !npm_resolver.is_pkg_req_folder_cached(&req) {
            diagnostics.push(DenoDiagnostic::NoCacheNpm(req, specifier.clone()));
          }
        }
