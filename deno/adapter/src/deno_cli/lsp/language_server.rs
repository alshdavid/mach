...
use crate::deno_cli::args::resolve_import_map;
...
use crate::deno_cli::cache::DenoDir;
...
use crate::deno_cli::lsp::logging::init_log_file;
use crate::deno_cli::lsp::tsc::file_text_changes_to_workspace_edit;
use crate::deno_cli::lsp::urls::LspUrlKind;
use crate::deno_cli::npm::create_cli_npm_resolver_for_lsp;
...
use crate::deno_cli::tools::fmt::format_file;
use crate::deno_cli::tools::fmt::format_parsed_source;
use crate::deno_cli::tools::upgrade::check_for_upgrades_for_lsp;
use crate::deno_cli::tools::upgrade::upgrade_check_enabled;
use crate::deno_cli::util::fs::remove_dir_all_if_exists;
...
impl LanguageServer {
  ...
  pub async fn cache(
    ...
    async fn create_graph_for_caching(
      ...
      let mut loader = crate::deno_cli::lsp::documents::OpenDocumentsGraphLoader {
    ...
    if let Some(result) = maybe_prepare_cache_result {
      ...
      let handle =
        spawn(async move { create_graph_for_caching(cli_options, roots, open_docs).await });
  ...
  pub async fn test_run_request(
    ...
    if let Some(testing_server) = &inner.maybe_testing_server {
      match params.map(serde_json::from_value) {
        Some(Ok(params)) => {
          testing_server.run_request(params, inner.config.workspace_settings().clone())
        }
  ...
  pub async fn virtual_text_document(
    ...
    match params.map(serde_json::from_value) {
      Some(Ok(params)) => Ok(Some(
        serde_json::to_value(self.0.read().await.virtual_text_document(params)?).map_err(
          |err| {
            error!(
    ...
    if capable {
      ...
      if let Ok(configs) = configs {
        ...
        for (folder_uri, _) in &folders {
          by_workspace_folder.insert(folder_uri.clone(), configs.next().unwrap());
        }
...
impl Inner {
  fn new(client: Client) -> Self {
    ...
    let module_registries =
      ModuleRegistry::new(module_registries_location.clone(), http_client.clone());
    ...
    let npm_search_api = CliNpmSearchApi::new(module_registries.file_fetcher.clone());
    let documents = Documents::new(deps_http_cache.clone());
    let cache_metadata = cache::CacheMetadata::new(deps_http_cache.clone());
    let performance = Arc::new(Performance::default());
    let ts_server = Arc::new(TsServer::new(performance.clone(), deps_http_cache.clone()));
  ...
  pub async fn get_navigation_tree(
    ...
    let mark = self
      .performance
      .mark_with_args("lsp.get_navigation_tree", json!({ "specifier": specifier }));
    ...
    let navigation_tree = if let Some(navigation_tree) = asset_or_doc.maybe_navigation_tree() {
      ...
      match asset_or_doc {
        ...
        AssetOrDocument::Document(doc) => self.documents.try_cache_navigation_tree(
          specifier,
          &doc.script_version(),
  ...
  fn get_config_file(&self) -> Result<Option<ConfigFile>, AnyError> {
    ...
    if let Some(root_uri) = self.config.root_uri() {
      ...
      let maybe_config = ConfigFile::discover_from(&root_path, &mut checked, None)?;
  ...
  fn get_package_json(
    ...
    if crate::deno_cli::args::has_flag_env_var("DENO_NO_PACKAGE_JSON") {
      return Ok(None);
    }
    ...
    let maybe_global_cache_path = if let Some(cache_str) = maybe_cache {
      ...
      let cache_url = if let Ok(url) = Url::from_file_path(cache_str) {
        ...
        Url::from_file_path(cache_path)
          .map_err(|_| anyhow!("Bad file path for import path: {:?}", cache_str))
      } else {
        Err(anyhow!(
  ...
  async fn recreate_http_client_and_dependents(&mut self) -> Result<(), AnyError> {
    self
  ...
  async fn set_new_global_cache_path(
    ...
    self.module_registries =
      ModuleRegistry::new(module_registries_location.clone(), self.http_client.clone());
    ...
    let maybe_local_cache = self
      .config
      .maybe_vendor_dir_path()
      .map(|local_path| Arc::new(LocalLspHttpCache::new(local_path, global_cache.clone())));
  ...
  pub async fn update_import_map(&mut self) -> Result<(), AnyError> {
    ...
    let maybe_import_map = self
      .fetch_import_map(maybe_import_map_url.as_ref(), CacheSetting::RespectHeaders)
      .await?;
  ...
  fn resolve_import_map_specifier(&self) -> Result<Option<ModuleSpecifier>, AnyError> {
    ...
    if let Ok(url) = Url::parse(&import_map_str) {
      ...
      let import_map_url = Url::from_file_path(import_map_path)
        .map_err(|_| anyhow!("Bad file path for import map: {}", import_map_str))?;
  ...
  async fn update_config_file(&mut self) -> Result<(), AnyError> {
    ...
    if let Some(config_file) = self.get_config_file()? {
      ...
      let fmt_options = config_file
        .to_fmt_config()
        .and_then(|maybe_fmt_config| FmtOptions::resolve(maybe_fmt_config, None, &self.initial_cwd))
      ...
      if let Some(config_file) = self.config.maybe_config_file() {
        if let Ok((compiler_options, _)) = config_file.to_compiler_options() {
          if let Some(compiler_options_obj) = compiler_options.as_object() {
            if let Some(jsx_import_source) = compiler_options_obj.get("jsxImportSource") {
  ...
  fn update_package_json(&mut self) -> Result<(), AnyError> {
    ...
    self.maybe_package_json = self.get_package_json(self.config.maybe_config_file())?;
    Ok(())
  }
  ...
  create_cli_npm_resolver_for_lsp(if is_byonm {
    ...
    CliNpmResolverCreateOptions::Managed(CliNpmResolverManagedCreateOptions {
      ...
      snapshot: match maybe_lockfile {
        Some(lockfile) => {
          CliNpmResolverManagedSnapshotOption::ResolveFromLockfile(lockfile.clone())
        }
      ...
      npm_registry_url: crate::deno_cli::args::npm_registry_url().to_owned(),
...
impl Inner {
  ...
  async fn refresh_npm_specifiers(&mut self) {
    ...
    let handle = spawn(async move {
      if let Some(npm_resolver) = npm_resolver.as_ref().and_then(|r| r.as_managed()) {
    ...
    fn has_lockfile_content_changed(lockfile: &Lockfile) -> bool {
      match Lockfile::new(lockfile.filename.clone(), false) {
        Ok(new_lockfile) => {
          ...
          FastInsecureHasher::hash(lockfile) != FastInsecureHasher::hash(new_lockfile)
        }
        Err(err) => {
    ...
    fn has_config_changed(
      ...
      match config.maybe_config_file() {
        ...
        None => {
          ...
          if changes
            .iter()
            .any(|url| url.path().ends_with("/deno.json") || url.path().ends_with("/deno.jsonc"))
          {
            return true;
          }
        }
  ...
  async fn formatting(
    ...
    if specifier.scheme() != "file" && params.text_document.uri.scheme() == "file" {
  ...
  async fn hover(
    ...
    let hover = if let Some((_, dep, range)) =
      asset_or_doc.get_maybe_dependency(&params.text_document_position_params.position)
    {
  ...
  async fn code_action(
    ...
    if !fixable_diagnostics.is_empty() {
        ...
        match diagnostic.source.as_deref() {
          ...
          Some("deno") => {
            if diagnostic.code == Some(NumberOrString::String("no-cache".to_string()))
              || diagnostic.code == Some(NumberOrString::String("no-cache-jsr".to_string()))
              || diagnostic.code == Some(NumberOrString::String("no-cache-npm".to_string()))
            {
      ...
      if includes_no_cache {
        let no_cache_diagnostics = self.diagnostics_state.no_cache_diagnostics(&specifier);
        ...
        if uncached_deps.len() > 1 {
          code_actions.add_cache_all_action(&specifier, no_cache_diagnostics.to_owned());
        }
      }
  ...
  async fn code_lens(
    ...
    if settings.code_lens.test && self.config.specifier_enabled_for_test(&specifier) {
      if let Some(Ok(parsed_source)) = asset_or_doc.maybe_parsed_source() {
        code_lenses.extend(
          code_lens::collect_test(&specifier, parsed_source).map_err(|err| {
            error!(
  ...
  async fn completion(
    ...
    let language_settings = self.config.language_settings_for_specifier(&specifier);
    if !self.is_diagnosable(&specifier)
        ...
        let result = self
          ...
          .get_completion_details(
            ...
            GetCompletionDetailsArgs {
              ...
              preferences: Some(tsc::UserPreferences::from_config_for_specifier(
                &self.config,
                &self.fmt_options.options,
                specifier,
              )),
  ...
  async fn incoming_calls(
    ...
    for item in incoming_calls.iter() {
      if let Some(resolved) =
        item.try_resolve_call_hierarchy_incoming_call(self, maybe_root_path_owned.as_deref())
      {
    ...
    let response = if let Some(one_or_many) = maybe_one_or_many {
      ...
      match one_or_many {
        tsc::OneOrMany::One(item) => {
          if let Some(resolved) =
            item.try_resolve_call_hierarchy_item(self, maybe_root_path_owned.as_deref())
          {
            resolved_items.push(resolved)
          }
        }
  ...
  async fn will_rename_files(
    ...
    for rename in params.files {
      let old_specifier = self
        .url_map
        .normalize_url(&resolve_url(&rename.old_uri).unwrap(), LspUrlKind::File);
      let options = self
        .config
      ...
      changes.extend(
        self
          ...
          .get_edits_for_file_rename(
            ...
            self
              .url_map
              .normalize_url(&resolve_url(&rename.new_uri).unwrap(), LspUrlKind::File),
            (&self.fmt_options.options).into(),
            tsc::UserPreferences {
...
impl Inner {
    ...
    let mut cli_options = CliOptions::new(
      Flags {
        ...
        unsafely_ignore_certificate_errors: workspace_settings
          .unsafely_ignore_certificate_errors
          .clone(),
        node_modules_dir: Some(self.config.maybe_node_modules_dir_path().is_some()),
        // bit of a hack to force the lsp to cache the @types/node package
        type_check_mode: crate::deno_cli::args::TypeCheckMode::Local,
        ..Default::default()
      },
  ...
  async fn inlay_hint(
    ...
    let line_index = asset_or_doc.line_index();
    let text_span =
      tsc::TextSpan::from_range(&params.range, line_index.clone()).map_err(|err| {
  ...
  fn virtual_text_document(
    ...
    let specifier = self
      .url_map
      .normalize_url(&params.text_document.uri, LspUrlKind::File);
    let contents = if specifier.scheme() == "deno" && specifier.path() == "/status.md" {
