...
fn as_delete_notification(uri: ModuleSpecifier) -> TestingNotification {
  TestingNotification::DeleteModule(lsp_custom::TestModuleDeleteNotificationParams {
...
impl TestServer {
          ...
          match update_rx.recv().await {
            ...
            Some(snapshot) => {
                ...
                .documents(DocumentsFilter::AllDiagnosable)
              {
                ...
                if !valid {
                  if let Some(Ok(parsed_source)) = document.maybe_parsed_source() {
                    let was_empty = tests
                      .remove(specifier)
                      .map(|tm| tm.is_empty())
                      .unwrap_or(true);
                    ...
                    if !test_module.is_empty() {
                      client
                        .send_test_notification(test_module.as_replace_notification(mru.as_ref()));
                    } else if !was_empty {
                      client.send_test_notification(as_delete_notification(specifier.clone()));
                    }
  ...
  pub fn run_request(
    ...
    let test_run = { TestRun::new(&params, self.tests.clone(), workspace_settings) };
