...
use crate::deno_cli::lsp::analysis::source_range_to_lsp_range;
    ...
    match expr {
      ast::Expr::Object(obj_lit) => {
        ...
        for prop in &obj_lit.props {
          ...
          let ast::PropName::Ident(ast::Ident { sym, .. }) = &key_value_prop.key else {
            continue;
          };
          if sym == "name" {
        ...
        for prop in &obj_lit.props {
          ...
          match prop.as_ref() {
            ...
            ast::Prop::Method(method_prop) => {
              let ast::PropName::Ident(ast::Ident { sym, .. }) = &method_prop.key else {
                continue;
              };
              if sym == "fn" {
    ...
    match callee_expr.as_ref() {
      ...
      ast::Expr::Member(member_expr) => {
        ...
        if ns_prop_ident_name == "ignore" || ns_prop_ident_name == "only" {
          let ast::Expr::Member(child_member_expr) = member_expr.obj.as_ref() else {
            return;
          };

          let ast::MemberProp::Ident(child_ns_prop_ident) = &child_member_expr.prop else {
            return;
          };
