...
use crate::deno_cli::lsp::cache::CacheMetadata;
use crate::deno_cli::lsp::documents::Documents;
use crate::deno_cli::lsp::logging::lsp_warn;
use crate::deno_cli::tsc;
use crate::deno_cli::tsc::ResolveArgs;
...
fn normalize_diagnostic(
  diagnostic: &mut crate::deno_cli::tsc::Diagnostic,
...
impl TsServer {
  ...
  pub async fn get_diagnostics(
  ...
  ) -> Result<HashMap<String, Vec<crate::deno_cli::tsc::Diagnostic>>, AnyError> {
    let req = TscRequest {
    ...
    let raw_diagnostics = self
      .request_with_cancellation::<HashMap<String, Vec<crate::deno_cli::tsc::Diagnostic>>>(
        snapshot, req, token,
      )
      .await?;
...
impl DocumentSpan {
  pub fn to_link(
    ...
    let target_asset_or_doc = language_server.get_maybe_asset_or_document(&target_specifier)?;
  ...
  fn to_target(
    ...
    let asset_or_doc = language_server.get_maybe_asset_or_document(&specifier)?;
...
impl NavigateToItem {
  pub fn to_symbol_information(
    ...
    let specifier = resolve_url(&self.file_name).ok()?;
    let asset_or_doc = language_server.get_asset_or_document(&specifier).ok()?;
    let line_index = asset_or_doc.line_index();
...
impl ImplementationLocation {
  ...
  pub fn to_location(
    ...
    let uri = language_server
      .url_map
      .normalize_specifier(&specifier)
      .unwrap_or_else(|_| LspClientUrl::new(ModuleSpecifier::parse("deno://invalid").unwrap()));
    lsp::Location {
      uri: uri.into_url(),
...
impl RenameLocations {
  pub async fn into_workspace_edit(
    ...
    let mut text_document_edit_map: HashMap<LspClientUrl, lsp::TextDocumentEdit> = HashMap::new();
...
impl Classifications {
  pub fn to_semantic_tokens(
    ...
    for i in 0..token_count {
      ...
      if start_pos.line == end_pos.line && start_pos.character <= end_pos.character {
        builder.push(
          start_pos.line,
          start_pos.character,
...
impl RefactorActionInfo {
  ...
  pub fn is_preferred(
    &self,
    all_actions: &[RefactorActionInfo],
  ) -> bool {
...
impl ApplicableRefactorInfo {
  pub fn to_code_actions(
    ...
    for action in self.actions.iter() {
      code_actions.push(self.as_inline_code_action(action, specifier, range, &self.name));
    }
    code_actions
  }
...
impl ReferenceEntry {
  pub fn to_location(
    ...
    let specifier =
      resolve_url(&self.document_span.file_name).unwrap_or_else(|_| INVALID_SPECIFIER.clone());
    let uri = url_map
      .normalize_specifier(&specifier)
...
impl CallHierarchyItem {
  ...
  pub fn to_call_hierarchy_item(
    ...
    let target_specifier = resolve_url(&self.file).unwrap_or_else(|_| INVALID_SPECIFIER.clone());
    let uri = language_server
      .url_map
      .normalize_specifier(&target_specifier)
      .unwrap_or_else(|_| LspClientUrl::new(INVALID_SPECIFIER.clone()));
...
impl CompletionEntryDetails {
  ...
  pub fn as_completion_item(
    ...
    let (command, additional_text_edits) =
      parse_code_actions(self.code_actions.as_ref(), data, specifier, language_server)?;
  ...
  pub fn as_completion_item(
    ...
    if let Some(source) = &self.source {
      ...
      if let Some(data) = &self.data {
        if let Ok(import_data) = serde_json::from_value::<CompletionEntryDataImport>(data.clone()) {
          if let Ok(import_specifier) = resolve_url(&import_data.file_name) {
              ...
              .or_else(|| relative_specifier(specifier, &import_specifier))
            {
              ...
              if new_module_specifier != import_data.module_specifier {
                specifier_rewrite = Some((import_data.module_specifier, new_module_specifier));
              }
...
impl SignatureHelpItem {
  pub fn into_signature_information(
    ...
    let prefix_text = display_parts_to_string(&self.prefix_display_parts, language_server);
    let params_text = self
      .parameters
      .iter()
      .map(|param| display_parts_to_string(&param.display_parts, language_server))
      .collect::<Vec<String>>()
      .join(", ");
    let suffix_text = display_parts_to_string(&self.suffix_display_parts, language_server);
    let documentation = display_parts_to_string(&self.documentation, language_server);
    lsp::SignatureInformation {
...
fn op_load<'s>(
  ...
  let maybe_load_response = if specifier.as_str() == "internal:///missing_dependency.d.ts" {
    None
  } else {
    let asset_or_document = state.get_asset_or_document(&specifier);
    asset_or_document.map(|doc| LoadResponse {
      data: doc.text(),
      script_kind: crate::deno_cli::tsc::as_ts_script_kind(doc.media_type()),
...
impl UserPreferences {
  pub fn from_config_for_specifier(
    ...
    let base_preferences = Self {
      ...
      include_completions_with_snippet_text: Some(config.client_capabilities.snippet_support),
...
fn request(
  ...
  let mark =
    performance.mark_with_args(format!("tsc.host.{}", request.method), request.args.clone());
  assert!(
