...
use crate::deno_cli::args::CoverageFlags;
use crate::deno_cli::args::FileFlags;
use crate::deno_cli::args::Flags;
use crate::deno_cli::cdp;
use crate::deno_cli::factory::CliFactory;
use crate::deno_cli::npm::CliNpmResolver;
use crate::deno_cli::tools::fmt::format_json;
use crate::deno_cli::tools::test::is_supported_test_path;
use crate::deno_cli::util::fs::FileCollector;
use crate::deno_cli::util::text_encoding::source_map_from_code;
...
impl crate::deno_cli::worker::CoverageCollector for CoverageCollector {
  ...
  async fn stop_collecting(&mut self) -> Result<(), AnyError> {
    ...
    for script_coverage in script_coverages {
      ...
      let formatted_coverage = format_json(&filepath, &coverage, &Default::default())
        .ok()
        .flatten()
        .unwrap_or(coverage);
...
fn generate_coverage_report(
  ...
  for function in &script_coverage.functions {
    ...
    let line_index = range_to_src_line_index(&function.ranges[0], &text_lines, &maybe_source_map);
  ...
  for (block_number, function) in script_coverage.functions.iter().enumerate() {
    ...
    for (branch_number, range) in function.ranges[1..].iter().enumerate() {
      let line_index = range_to_src_line_index(range, &text_lines, &maybe_source_map);
  ...
  for line_index in 0..text_lines.lines_count() {
    ...
    let ignore = comment_ranges
      .iter()
      .any(|range| range.start <= line_start_byte_offset && range.end >= line_end_byte_offset)
      || script_source[line_start_byte_offset..line_end_byte_offset]
        .trim()
        .is_empty();
    let mut count = 0;
...
pub async fn cover_files(
  ...
  let script_coverages = collect_coverages(coverage_flags.files, cli_options.initial_cwd())?;
  ...
  for script_coverage in script_coverages {
    let module_specifier =
      deno_core::resolve_url_or_path(&script_coverage.url, cli_options.initial_cwd())?;
    ...
    let maybe_file = if module_specifier.scheme() == "file" {
      ...
      file_fetcher
        ...
        .with_context(|| format!("Failed to fetch \"{module_specifier}\" from cache."))?
    };
