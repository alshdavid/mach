...
use crate::deno_cli::args::InfoFlags;
...
use crate::deno_cli::npm::ManagedCliNpmResolver;
...
pub async fn info(
  ...
  if let Some(specifier) = info_flags.file {
    ...
    let maybe_import_specifier = if let Some(imports_map) = maybe_imports_map {
      if let Ok(imports_specifier) = imports_map.resolve(&specifier, imports_map.base_url()) {
...
fn print_cache_info(
  ...
  if let Some(location) = &location {
    origin_dir = origin_dir.join(checksum::gen(&[location.to_string().as_bytes()]));
  }
...
fn add_npm_packages_to_json(
  ...
  if let Some(modules) = modules {
    if modules.len() == 1 && modules[0].get("kind").and_then(|k| k.as_str()) == Some("npm") {
      ...
      if let Some(pkg) = maybe_package {
        if let Some(module) = module.as_object_mut() {
          module.insert("npmPackage".to_string(), pkg.id.as_serialized().into());
        }
      }
    } else {
            ...
            if let Some(specifier) = specifier {
              if let Ok(npm_ref) = NpmPackageReqReference::from_str(specifier) {
                if let Ok(pkg) = snapshot.resolve_pkg_from_pkg_req(npm_ref.req()) {
                  dep.insert("npmPackage".to_string(), pkg.id.as_serialized().into());
                }
              }
            }
          }
  ...
  let mut sorted_packages = snapshot.all_packages_for_every_system().collect::<Vec<_>>();
...
impl<'a> GraphDisplayContext<'a> {
      ...
      Ok(Some(root)) => {
        ...
        if let Some(cache_info) = maybe_cache_info {
          ...
          if let Some(map) = &cache_info.map {
            writeln!(writer, "{} {}", colors::bold("map:"), map.to_string_lossy())?;
          }
        }
        ...
        Specifier(_) => {
          if let Some(module) = module.js() {
            if let Some(types_dep) = &module.maybe_types_dependency {
              if let Some(child) = self.build_resolved_info(&types_dep.dependency, true) {
  ...
  fn build_npm_deps(
    ...
    for dep_id in deps.into_iter() {
      ...
      let mut child = TreeNode::from_text(format!("npm:/{} {}", dep_id.as_serialized(), size_str));
  ...
  fn build_error_info(
    ...
    match err {
      ...
      ModuleError::LoadingErr(_, _, _) => self.build_error_msg(specifier, "(loading error)"),
      ModuleError::ParseErr(_, _) => self.build_error_msg(specifier, "(parsing error)"),
      ModuleError::UnsupportedImportAttributeType { .. } => {
        self.build_error_msg(specifier, "(unsupported import attribute)")
      }
      ModuleError::UnsupportedMediaType { .. } => self.build_error_msg(specifier, "(unsupported)"),
      ...
      ModuleError::UnknownExport { .. } => self.build_error_msg(specifier, "(unknown export)"),
      ModuleError::UnknownPackage { .. } => self.build_error_msg(specifier, "(unknown package)"),
