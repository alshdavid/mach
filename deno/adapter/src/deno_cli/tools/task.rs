...
use crate::deno_cli::args::TaskFlags;
...
pub async fn execute_script(
  ...
  if let Some(script) = tasks_config.get(task_name) {
    ...
    let future = deno_task_shell::execute(seq_list, env_vars, &cwd, Default::default());
...
fn collect_env_vars_with_node_modules_dir(node_modules_dir_path: &Path) -> HashMap<String, String> {
  ...
  match env_vars.get_mut("PATH") {
    Some(path) => {
      if path.is_empty() {
        ...
        *path = format!("{}{}{}", value, if cfg!(windows) { ";" } else { ":" }, path);
      }
    }
    None => {
...
fn collect_env_vars() -> HashMap<String, String> {
  ...
  if !env_vars.contains_key(INIT_CWD_NAME) {
    if let Ok(cwd) = std::env::current_dir() {
      ...
      env_vars.insert(INIT_CWD_NAME.to_string(), cwd.to_string_lossy().to_string());
    }
  }
  env_vars
}
...
impl ShellCommand for NpmPackageBinCommand {
  fn execute(
    ...
    let executable_command = deno_task_shell::ExecutableCommand::new("deno".to_string());
