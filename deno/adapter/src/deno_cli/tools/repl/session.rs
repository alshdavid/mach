...
use crate::deno_cli::lsp::ReplLanguageServer;
use crate::deno_cli::npm::CliNpmResolver;
use crate::deno_cli::resolver::CliGraphResolver;
use crate::deno_cli::tools::test::report_tests;
use crate::deno_cli::tools::test::reporters::PrettyTestReporter;
use crate::deno_cli::tools::test::reporters::TestReporter;
use crate::deno_cli::tools::test::run_tests_for_worker;
use crate::deno_cli::tools::test::worker_has_tests;
use crate::deno_cli::tools::test::TestEvent;
use crate::deno_cli::tools::test::TestEventReceiver;
use crate::deno_cli::tools::test::TestEventSender;
...
impl ReplSession {
  pub async fn initialize(
    ...
    let emit_options =
      crate::deno_cli::args::ts_config_to_emit_options(ts_config_for_emit.ts_config);
  ...
  async fn evaluate_ts_expression(
    ...
    let parsed_source = match parse_source_as(expression.to_string(), deno_ast::MediaType::Tsx) {
      Ok(parsed) => parsed,
      Err(err) => {
        if let Ok(parsed) = parse_source_as(expression.to_string(), deno_ast::MediaType::TypeScript)
        {
          parsed
        } else {
          return Err(err);
    ...
    if let Some(captures) = JSX_IMPORT_SOURCE_RE.captures(&c.text) {
      if let Some(m) = captures.get(1) {
        analyzed_pragmas.jsx_import_source = Some(SpecifierWithRange {
          ...
          range: comment_source_to_position_range(c.start(), &m, parsed_source.text_info(), true),
        });
      }
    }
    ...
    if let Some(captures) = JSX_RE.captures(&c.text) {
      if let Some(m) = captures.get(1) {
        analyzed_pragmas.jsx = Some(SpecifierWithRange {
          text: m.as_str().to_string(),
          range: comment_source_to_position_range(c.start(), &m, parsed_source.text_info(), false),
        });
      }
    }
