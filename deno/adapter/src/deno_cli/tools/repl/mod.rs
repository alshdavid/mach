...
use crate::deno_cli::args::ReplFlags;
...
async fn read_eval_file(
  ...
  let specifier = deno_core::resolve_url_or_path(eval_file, cli_options.initial_cwd())?;
...
pub async fn run(
  ...
  let mut worker = worker_factory
    .create_custom_worker(
      main_module.clone(),
      permissions,
      vec![crate::deno_cli::ops::testing::deno_test::init_ops(
  ...
  if !cli_options.is_quiet() {
    println!("Deno {}", crate::deno_cli::version::deno());
