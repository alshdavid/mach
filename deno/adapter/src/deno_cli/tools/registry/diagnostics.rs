  ...
  pub fn print_and_error(&self) -> Result<(), AnyError> {
    ...
    if errors > 0 {
      if has_slow_types_errors {
        eprintln!("This package contains errors for slow types. Fixing these errors will:\n");
        eprintln!("  1. Significantly improve your package users' type checking performance.");
...
impl Diagnostic for PublishDiagnostic {
  ...
  fn hint(&self) -> Option<Cow<'_, str>> {
    match &self {
      ...
      PublishDiagnostic::InvalidPath { .. } => Some(Cow::Borrowed(
        "rename or remove the file, or add it to 'publish.exclude' in the config file",
      )),
      PublishDiagnostic::DuplicatePath { .. } => Some(Cow::Borrowed("rename or remove the file")),
      PublishDiagnostic::UnsupportedFileType { .. } => Some(Cow::Borrowed(
        "remove the file, or add it to 'publish.exclude' in the config file",
      )),
      PublishDiagnostic::InvalidExternalImport { .. } => Some(Cow::Borrowed(
