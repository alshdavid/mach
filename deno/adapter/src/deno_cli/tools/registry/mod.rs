...
use crate::deno_cli::args::jsr_api_url;
use crate::deno_cli::args::jsr_url;
use crate::deno_cli::args::CliOptions;
use crate::deno_cli::args::Flags;
use crate::deno_cli::args::PublishFlags;
use crate::deno_cli::cache::LazyGraphSourceParser;
...
use crate::deno_cli::tools::check::CheckOptions;
use crate::deno_cli::tools::lint::no_slow_types;
use crate::deno_cli::tools::registry::diagnostics::PublishDiagnostic;
use crate::deno_cli::tools::registry::diagnostics::PublishDiagnosticsCollector;
use crate::deno_cli::tools::registry::graph::collect_invalid_external_imports;
use crate::deno_cli::util::display::human_size;
...
async fn get_auth_headers(
  ...
  match auth_method {
    AuthMethod::Interactive => {
      ...
      let auth = api::parse_response::<api::CreateAuthorizationResponse>(response)
        .await
        .context("Failed to create interactive authorization")?;
      ...
      loop {
        ...
        let res = api::parse_response::<api::ExchangeAuthorizationResponse>(response).await;
        match res {
          Ok(res) => {
            println!(
              "{} {} {}",
...
async fn check_if_scope_and_package_exist(
  ...
  let response = api::get_package(client, registry_api_url, scope, package).await?;
  if response.status() == 404 {
    needs_package = true;
  }
...
async fn ensure_scopes_and_packages_exist(
  ...
  for package in packages {
    ...
    let package_api_url =
      api::get_package_api_url(&registry_api_url, &package.scope, &package.package);
...
async fn perform_publish(
  ...
  let mut authorizations =
    get_auth_headers(client, registry_api_url.clone(), packages, auth_method).await?;
...
async fn publish_package(
  ...
  let mut task = match res {
    ...
    Err(mut err) if err.code == "duplicateVersionPublish" => {
      let task =
        serde_json::from_value::<api::PublishingTask>(err.data.get_mut("task").unwrap().take())
          .unwrap();
...
async fn prepare_packages_for_publishing(
  ...
  let publish_order_graph = publish_order::build_publish_order_graph(&graph, &members)?;
  ...
  let results = members
    ...
    .map(|member| {
      ...
      async move {
        ...
        .await
        .with_context(|| format!("Failed preparing '{}'.", member.package_name))?;
...
pub async fn publish(
  ...
  let import_map = cli_factory
    ...
    .await?
    .clone()
    .unwrap_or_else(|| Arc::new(ImportMap::new(Url::parse("file:///dev/null").unwrap())));
