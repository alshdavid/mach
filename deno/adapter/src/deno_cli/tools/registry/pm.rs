...
use crate::deno_cli::args::AddFlags;
...
pub async fn add(
  ...
  for package_name in add_flags.packages.iter() {
    let req = if package_name.starts_with("npm:") {
      let pkg_req = NpmPackageReqReference::from_str(package_name)
        .with_context(|| format!("Failed to parse package required: {}", package_name))?;
  ...
  while let Some(package_and_version_result) = buffered.next().await {
    ...
    match package_and_version {
      PackageAndVersion::NotFound(package_name) => {
        bail!(
          "{} was not found.",
          crate::deno_cli::colors::red(package_name)
        );
      }
  ...
  let config_file_contents = tokio::fs::read_to_string(&config_file_path).await.unwrap();
  ...
  let mut existing_imports = if let Some(imports) = config_file.json.imports.clone() {
  ...
  for selected_package in selected_packages {
    log::info!(
      "Add {} - {}@{}",
      crate::deno_cli::colors::green(&selected_package.import_name),
  ...
  let mut import_list: Vec<(String, String)> = existing_imports.into_iter().collect();
...
fn update_config_file_content(
  ...
  let new_text = deno_ast::apply_text_changes(config_file_contents, text_changes);

  crate::deno_cli::tools::fmt::format_json(&PathBuf::from("deno.json"), &new_text, &fmt_options)
    .ok()
