...
use crate::deno_cli::cache::TypeCheckCache;
use crate::deno_cli::graph_util::BuildFastCheckGraphOptions;
use crate::deno_cli::graph_util::ModuleGraphBuilder;
use crate::deno_cli::npm::CliNpmResolver;
use crate::deno_cli::tsc;
use crate::deno_cli::tsc::Diagnostics;
...
impl TypeChecker {
  ...
  pub async fn check_diagnostics(
    ...
    let maybe_check_hash = match self.npm_resolver.check_state_hash() {
      Some(npm_check_hash) => {
        match get_check_hash(&graph, npm_check_hash, type_check_mode, &ts_config) {
          CheckHashResult::NoFiles => return Ok((graph.into(), Default::default())),
...
fn get_tsc_roots(
  ...
  fn maybe_get_check_entry(
    ...
    match module {
      ...
      Module::External(_) | Module::Node(_) | Module::Npm(_) | Module::Json(_) => None,
    }
  }
  ...
  let mut seen = HashSet::with_capacity(graph.imports.len() + graph.specifiers_count());
