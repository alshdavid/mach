...
use crate::deno_cli::args::VendorFlags;
...
use crate::deno_cli::util::fs::resolve_from_cwd;
...
pub async fn vendor(
  ...
  let entry_points = resolve_entry_points(&vendor_flags, cli_options.initial_cwd())?;
  ...
  let try_add_node_modules_dir =
    npm_package_count > 0 && cli_options.node_modules_dir_enablement().unwrap_or(true);
  ...
  if modified_result.added_node_modules_dir {
    ...
    if let Some(node_modules_path) = node_modules_path {
      let cli_options = cli_options.with_node_modules_dir_path(node_modules_path);
      let factory = CliFactory::from_cli_options(Arc::new(cli_options));
...
fn validate_options(
    ...
    .and_then(|p| canonicalize_path(&p).ok())
  {
    ...
    let output_dir = canonicalize_path(output_dir)
...
fn maybe_update_config_file(
  ...
  let result = update_config_file(
    ...
    if try_add_import_map {
      Some(ModuleSpecifier::from_file_path(output_dir.join("import_map.json")).unwrap())
    } else {
      None
    },
    try_add_node_modules_dir,
...
fn update_config_file(
  ...
  let import_map_specifier = import_map_specifier.and_then(|import_map_specifier| {
