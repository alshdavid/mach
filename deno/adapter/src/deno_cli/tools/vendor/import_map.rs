...
impl<'a> ImportMapBuilder<'a> {
  ...
  pub fn into_import_map(
    ...
    fn add_local_imports<'a>(
      ...
      let local_imports = get_local_imports(new_relative_path, original_imports);
    ...
    if let Some(original_im) = original_import_map {
      ...
      for scope in original_im.scopes() {
        if scope.raw_key.starts_with("./") || scope.raw_key.starts_with("../") {
          ...
          let new_key = format!("{}{}", new_relative_path, &scope.raw_key[sub_index..]);
...
pub fn build_import_map(input: BuildImportMapInput<'_>) -> Result<String, AnyError> {
...
fn visit_modules(
  ...
  for module in modules {
    let module = match module {
      ...
      Module::Json(_) | Module::Npm(_) | Module::Node(_) | Module::External(_) => continue,
    };

    let parsed_source = parsed_source_cache.get_parsed_source_from_js_module(module)?;
...
fn handle_remote_dep_specifier(
  ...
  if is_remote_specifier_text(text) {
    ...
    if text.starts_with(base_specifier.as_str()) {
      ...
      let relative_text = mappings.relative_specifier_text(base_specifier, specifier);
    ...
    let expected_relative_specifier_text = mappings.relative_specifier_text(referrer, specifier);
      ...
      imports.add(
        {
          ...
          let mut value =
            ModuleSpecifier::from_directory_path(local_path.parent().unwrap()).unwrap();
...
fn handle_local_dep_specifier(
  ...
  if text.starts_with("./") || text.starts_with("../") {
    ...
    let mut specifier_local_uri = referrer_local_uri.join(text).unwrap_or_else(|_| {
      panic!(
        "Error joining {} to {}",
        unresolved_specifier.path(),
        referrer_local_uri
