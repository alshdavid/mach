...
impl DotTestReporter {
  pub fn new() -> DotTestReporter {
    let console_width = if let Some(size) = crate::deno_cli::util::console::console_size() {
      size.cols as usize
    } else {
      0
    };
...
impl TestReporter for DotTestReporter {
  ...
  fn report_summary(
    ...
    common::report_summary(&mut std::io::stdout(), &self.cwd, &self.summary, elapsed);
    println!();
  }
