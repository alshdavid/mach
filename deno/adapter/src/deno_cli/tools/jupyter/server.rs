...
impl JupyterServer {
  pub async fn start(
    ...
    let handle2 = deno_core::unsync::spawn(async move {
      if let Err(err) = Self::handle_control(control_socket, cancel_handle2).await {
      ...
      "complete_request" => {
        ...
        if !lsp_completions.is_empty() {
          ...
          let (completions, cursor_start) = if let Some(index) = expr.rfind('.') {
            ...
            let candidates = get_expression_property_names(&mut self.repl_session, sub_expr)
              .await
              .into_iter()
              .filter(|n| {
            ...
            let mut candidates =
              get_expression_property_names(&mut self.repl_session, "globalThis")
                .await
                .into_iter()
  ...
  async fn handle_execution_request(
    ...
    *self.last_execution_request.borrow_mut() = Some(msg.clone());
    ...
    if exception_details.is_none() {
      publish_result(&mut self.repl_session, &result, self.execution_count).await?;
...
// TODO(bartlomieju): dedup with repl::editor
async fn get_global_lexical_scope_names(session: &mut repl::ReplSession) -> Vec<String> {
...
async fn evaluate_expression(
  ...
  let evaluate_response = session
    ...
    .await
    .ok()?;
  let evaluate_response: cdp::EvaluateResponse = serde_json::from_value(evaluate_response).ok()?;
