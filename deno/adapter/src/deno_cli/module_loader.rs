...
use crate::deno_cli::graph_util::CreateGraphOptions;
...
use crate::deno_cli::resolver::ModuleCodeStringSource;
...
use crate::deno_cli::util::text_encoding::code_without_source_map;
  ...
  fn collect_specifiers(
    ...
    Ok(
      files
        .iter()
        .filter_map(|file| {
          let file_url = resolve_url_or_path(file, self.options.initial_cwd()).ok()?;
...
impl PreparedModuleLoader {
  pub fn load_prepared_module(
    ...
    match graph.get(specifier) {
      ...
      Some(
        deno_graph::Module::External(_) | deno_graph::Module::Node(_) | deno_graph::Module::Npm(_),
      )
      | None => {
...
impl CliModuleLoader {
  ...
  fn inner_resolve(
    ...
    let maybe_resolved = match graph.get(referrer) {
      Some(Module::Js(module)) => module.dependencies.get(specifier).map(|d| &d.maybe_code),
      _ => None,
    };
  ...
  fn get_source_line(
    ...
    if line_number >= lines.len() {
      Some(format!(
        ...
        crate::deno_cli::colors::yellow("Warning"), line_number + 1,
      ))
    } else {
