...
impl NpmResolution {
  pub fn from_serialized(
    ...
    let snapshot = NpmResolutionSnapshot::new(initial_snapshot.unwrap_or_default());
  ...
  pub async fn add_package_reqs(
    ...
    let snapshot =
      add_package_reqs_to_snapshot(&self.api, package_reqs, self.maybe_lockfile.clone(), || {
  ...
  pub async fn resolve_pending(&self) -> Result<(), AnyError> {
    ...
    let snapshot =
      add_package_reqs_to_snapshot(&self.api, &Vec::new(), self.maybe_lockfile.clone(), || {
        self.snapshot.read().clone()
      })
      .await?;
  ...
  pub fn resolve_pkg_req_as_pending_with_info(
    ...
    let nv =
      pending_resolver.resolve_package_req_as_pending(&mut snapshot, pkg_req, package_info)?;
    Ok(nv)
  }
  ...
  pub fn serialized_valid_snapshot(&self) -> ValidSerializedNpmResolutionSnapshot {
...
fn get_npm_pending_resolver(
  ...
  NpmResolutionSnapshotPendingResolver::new(NpmResolutionSnapshotPendingResolverOptions {
    api,
    // WARNING: When bumping this version, check if anything needs to be
    // updated in the `setNodeOnlyGlobalNames` call in 99_main_compiler.js
    types_node_version_req: Some(VersionReq::parse_from_npm("18.0.0 - 18.16.19").unwrap()),
  })
}
...
fn populate_lockfile_from_snapshot(
  ...
  for package in snapshot.all_packages_for_every_system() {
    lockfile.check_or_insert_npm_package(npm_package_to_lockfile_info(package))?;
  }
  Ok(())
}
