...
fn create_inner(
  ...
  text_only_progress_bar: crate::deno_cli::util::progress_bar::ProgressBar,
  ...
  let package_json_deps_installer = match package_json_installer {
    CliNpmResolverManagedPackageJsonInstallerOption::ConditionalInstall(provider) => Arc::new(
      PackageJsonDepsInstaller::new(provider, npm_api.clone(), resolution.clone()),
    ),
...
impl ManagedCliNpmResolver {
  ...
  pub fn package_size(
    ...
    Ok(crate::deno_cli::util::fs::dir_size(&package_folder)?)
  }
  ...
  pub fn is_pkg_req_folder_cached(
    &self,
    req: &PackageReq,
  ) -> bool {
    self
  ...
  pub async fn ensure_top_level_package_json_install(&self) -> Result<(), AnyError> {
    self
      .package_json_deps_installer
