...
use crate::deno_cli::args::package_json::get_local_package_json_version_reqs;
use crate::deno_cli::args::NpmProcessState;
use crate::deno_cli::args::NpmProcessStateKind;
...
impl ByonmCliNpmResolver {
  ...
  pub fn find_ancestor_package_json_with_dep(
    ...
    loop {
      ...
      if let Ok(pkg_json) = PackageJson::load_skip_read_permission(self.fs.as_ref(), pkg_json_path)
      {
...
impl NpmResolver for ByonmCliNpmResolver {
  fn get_npm_process_state(&self) -> String {
    serde_json::to_string(&NpmProcessState {
      ...
      local_node_modules_path: Some(self.root_node_modules_dir.to_string_lossy().to_string()),
    })
    .unwrap()
  }
    ...
    fn resolve_from_package_json(
      ...
      for (key, value) in deps {
        if let Ok(value) = value {
          if value.name == req.name && value.version_req.intersects(&req.version_req) {
    ...
    if let Ok(file_path) = specifier_to_file_path(referrer) {
      ...
      while let Some(dir_path) = current_path.parent() {
        ...
        if self.fs.exists_sync(&package_json_path) {
          return resolve_from_package_json(req, self.fs.as_ref(), package_json_path);
        }
