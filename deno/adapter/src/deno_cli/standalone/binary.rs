...
use crate::deno_cli::args::package_json::PackageJsonDepValueParseError;
...
pub fn extract_standalone(
  exe_path: &Path,
  cli_args: Vec<String>,
) -> Result<Option<impl Future<Output = Result<(Metadata, eszip::EszipV2), AnyError>>>, AnyError> {
  ...
  Ok(Some(async move {
    let bufreader = deno_core::futures::io::BufReader::new(AllowStdIo::new(file));
...
impl<'a> DenoCompileBinaryWriter<'a> {
  ...
  async fn get_base_binary(
    ...
    let binary_path_suffix = if crate::deno_cli::version::is_canary() {
      format!(
        "canary/{}/{}",
      ...
      format!("release/v{}/{}", "CARGO_PKG_VERSION", binary_name)
    };
  ...
  async fn write_standalone_binary(
    ...
    let metadata = Metadata {
      ...
      maybe_import_map,
      node_modules,
      disable_deprecated_api_warning: cli_options.disable_deprecated_api_warning,
