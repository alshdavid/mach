...
impl VfsBuilder {
          ...
          Ok(target) => {
            if let Err(StripRootError { .. }) = self.add_symlink(&path, &target) {
              if target.is_file() {
                ...
                let file_bytes =
                  std::fs::read(&target).with_context(|| format!("Reading {}", path.display()))?;
  ...
  pub fn add_file_at_path(
    ...
    let file_bytes = std::fs::read(path).with_context(|| format!("Reading {}", path.display()))?;
    self.add_file(path, file_bytes)
  }
  ...
  fn path_relative_root(
    &self,
    path: &Path,
  ) -> Result<PathBuf, StripRootError> {
...
impl FileBackedVfs {
  ...
  pub fn dir_entry(
    &self,
    path: &Path,
  ) -> std::io::Result<&VirtualDirectory> {
    let (_, entry) = self.fs_root.find_entry(path)?;
    match entry {
