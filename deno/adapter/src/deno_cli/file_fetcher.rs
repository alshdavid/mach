...
impl FileFetcher {
  ...
  async fn fetch_blob_url(
    ...
    let blob = self
      ...
      .ok_or_else(|| custom_error("NotFound", format!("Blob URL not found: \"{specifier}\".")))?;
  ...
  fn fetch_remote(
    ...
    if *cache_setting == CacheSetting::Only {
      return futures::future::err(custom_error(
        "NotCached",
        format!("Specifier not found in cache: \"{specifier}\", --cached-only is specified."),
      ))
      .boxed();
    }
  ...
  fn should_use_cache(
    ...
    match cache_setting {
      ...
      CacheSetting::RespectHeaders => {
        ...
        let cache_semantics = CacheSemantics::new(headers, download_time, SystemTime::now());
...
async fn fetch_once<'a>(
  ...
  let body =
    http_util::get_response_body_with_progress(response, args.maybe_progress_guard).await?;
