...
pub static LAZILY_LOADED_STATIC_ASSETS: Lazy<HashMap<&'static str, &'static str>> =
  Lazy::new(|| {
    ([
      (
...
fn op_load(
  ...
  // handle the request for that module here.
    ...
    let specifier = if let Some(remapped_specifier) = state.remapped_specifiers.get(load_specifier)
    {
      remapped_specifier
...
fn op_resolve(
  ...
  let referrer = if let Some(remapped_specifier) = state.remapped_specifiers.get(&args.base) {
    ...
    normalize_specifier(&args.base, &state.current_dir)
  ...
  match maybe_module {
    ...
    Some(Module::Npm(module)) => {
      if let Some(npm) = &state.maybe_npm.as_ref() {
        ...
        let maybe_resolution = npm.node_resolver.resolve_package_subpath_from_deno_module(
          &package_folder,
...
fn resolve_non_graph_specifier_types(
  ...
  if node_resolver.in_npm_package(referrer) {
    ...
    let maybe_resolution = node_resolver.resolve_package_subpath_from_deno_module(
      &package_folder,
