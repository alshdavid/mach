...
use crate::deno_cli::args::deno_json::deno_json_deps;
...
use crate::deno_cli::emit::Emitter;
use crate::deno_cli::file_fetcher::FileFetcher;
use crate::deno_cli::graph_util::FileWatcherReporter;
use crate::deno_cli::graph_util::ModuleGraphBuilder;
use crate::deno_cli::graph_util::ModuleGraphContainer;
use crate::deno_cli::graph_util::ModuleGraphCreator;
...
use crate::deno_cli::standalone::DenoCompileBinaryWriter;
use crate::deno_cli::tools::check::TypeChecker;
use crate::deno_cli::tools::coverage::CoverageCollector;
use crate::deno_cli::tools::run::hmr::HmrRunner;
...
impl CliFactory {
  ...
  pub fn maybe_lockfile(&self) -> &Option<Arc<Mutex<Lockfile>>> {
    fn check_no_npm(
      ...
      options
        .maybe_package_json()
        .as_ref()
        .map(|package_json| package_json.path.parent() != lockfile.lock().filename.parent())
        .unwrap_or(false)
    }
        ...
        let config = match self.options.maybe_workspace_config() {
          Some(workspace_config) => deno_lockfile::WorkspaceConfig {
            root: WorkspaceMemberConfig {
              package_json_deps,
              dependencies: deno_json_deps(self.options.maybe_config_file().as_ref().unwrap())
                .into_iter()
  ...
  pub fn emit_cache(&self) -> Result<&EmitCache, AnyError> {
    self
      .services
      .emit_cache
      .get_or_try_init(|| Ok(EmitCache::new(self.deno_dir()?.gen_cache.clone())))
  }
  ...
  pub fn emitter(&self) -> Result<&Arc<Emitter>, AnyError> {
    self.services.emitter.get_or_try_init(|| {
      ...
      let emit_options =
        crate::deno_cli::args::ts_config_to_emit_options(ts_config_result.ts_config);
  ...
  pub fn feature_checker(&self) -> &Arc<FeatureChecker> {
    self.services.feature_checker.get_or_init(|| {
      ...
      checker.set_warn_cb(Box::new(crate::deno_cli::unstable_warn_cb));
      ...
      for (flag_name, _, _) in crate::deno_cli::UNSTABLE_GRANULAR_FLAGS {
  ...
  fn create_cli_main_worker_options(&self) -> Result<CliMainWorkerOptions, AnyError> {
    let create_hmr_runner = if self.options.has_hmr() {
      ...
      let fn_: crate::deno_cli::worker::CreateHmrRunnerCb = Box::new(move |session| {
    ...
    let create_coverage_collector = if let Some(coverage_dir) = self.options.coverage_dir() {
      let coverage_dir = PathBuf::from(coverage_dir);
      let fn_: crate::deno_cli::worker::CreateCoverageCollectorCb =
        Box::new(move |session| Box::new(CoverageCollector::new(coverage_dir.clone(), session)));
      Some(fn_)
    } else {
      None
    };
