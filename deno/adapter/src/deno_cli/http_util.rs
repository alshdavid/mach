...
impl CacheSemantics {
  ...
  fn max_age(&self) -> Duration {
    ...
    if let Some(expires) = self.headers.get("expires") {
      return match DateTime::parse_from_rfc2822(expires) {
        ...
        Ok(expires) => {
          let expires =
            SystemTime::UNIX_EPOCH + Duration::from_secs(expires.timestamp().max(0) as _);
          return default_min_ttl.max(expires.duration_since(server_date).unwrap_or_default());
        }
      };
    }
    ...
    if let Some(last_modified) = self.headers.get("last-modified") {
      if let Ok(last_modified) = DateTime::parse_from_rfc2822(last_modified) {
        let last_modified =
          SystemTime::UNIX_EPOCH + Duration::from_secs(last_modified.timestamp().max(0) as _);
  ...
  fn raw_server_date(&self) -> SystemTime {
    self
      ...
      .and_then(|d| SystemTime::UNIX_EPOCH.checked_add(Duration::from_secs(d.timestamp() as _)))
