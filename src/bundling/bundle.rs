...
pub fn bundle(
  ...
  let source_map = source_map_ref.take();
  ...
  for asset in asset_map.iter() {
    let Asset::JavaScript(asset) = asset else { continue; };
    bundle.assets.push(asset.clone());
  ...
  'lookup: for (_, dependencies) in dependency_map.iter() {
    for dependency in dependencies.iter() {
      if dependency.parent_asset_id == "" {
        ...
        bundle.name = format!("{}-{}", &file_name[0..file_name.len() - file_extension.len() - 1], bundle.name);
        break 'lookup;
      }
    }
  }
  ...
  source_map_ref.insert(source_map);
  Ok(())
}
