...
if (
  JSON.parse(fs.readFileSync(path.join(__dirname, 'package.json'), 'utf8'))
    .version === '0.0.0-local' ||
  process.env.MACH_SKIP_POST_INSTALL === 'true'
) {
  process.exit(0)
}
...
if (process.env.MACH_BIN_OVERRIDE) {
  ...
  fs.linkSync(process.env.MACH_BIN_OVERRIDE, path.join(__dirname, 'bin.exe'))
  process.exit(0)
}
...
let bin_pkg_json_path = require.resolve(
  `@alshdavid/mach-${OS}-${ARCH}/package.json`,
)
